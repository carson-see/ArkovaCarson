...
CREATE OR REPLACE FUNCTION public.upsert_drive_watch_state(
  ...
  -- Owner-scope + provenance (DRIVE-02). owner_email / folder_path are SENSITIVE
  -- (customer hierarchy / actor identity) — org-scoped by RLS, never logged by
  -- the worker bootstrap module. created_by records which admin bootstrapped.
