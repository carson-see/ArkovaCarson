...
| `0379` | `fix/scrum-2904-envelope-lookup-statement-timeout` (this PR) | SCRUM-2904 follow-up (2026-08-01 prod incident, Sentry ARKOVA-WORKER-2B) | `0379_scrum2904_envelope_reconciliation_indexes.sql` | RESERVED — pre-soak, file-only, NOT applied to prod/rig. T3 (touches `supabase/migrations/` + indexes `anchors`, anchor-lifecycle path). Three `CREATE INDEX CONCURRENTLY` partial expression indexes on `anchors (org_id, (metadata->>'<key>'), created_at) WHERE deleted_at IS NULL AND (metadata->>'<key>') IS NOT NULL`, one per envelope-id metadata key (`source_envelope_id` / `envelope_id` / `external_ref`). Claimed as `max(main head 0378, this table, open-PR migrations)+1` — `0378` is the prod-applied numeric ledger head, and an open-PR sweep this session (`gh pr view --json files` over all 22 open PRs) found migration claims only at `0375` (#1739), `0364` (#1652), `0362` (#1618), so `0379` is the next free slot. **CONCURRENTLY — no BEGIN/COMMIT in the file**; apply outside a txn and verify all three `indisvalid`. Paired with the worker-side split of `findExistingEnvelopeAnchor` from one unindexed `.or()` into one `.eq()` query per key |
