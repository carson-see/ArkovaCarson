...
## Lane 1 migration reservation (2026-07-21) — SCRUM-2917 (PR pending)

Reserved by the Lane-1 SCRUM-2917 proof-materializer slice per the CTO ruling (Confluence 110198785) and the SCRUM-2979 prefix reservation. **FILE-ONLY / PRE-SOAK / NOT APPLIED anywhere (no prod, no staging, no rig); do-not-apply until the RTE rig is up and an explicit go is given.** Both land strictly AFTER `0358` (unmerged chain-rail PR #1552). Do not reserve or reuse `0358`, `0359`, or `0360`.

| Prefix | Branch / PR | Story | File | Status |
|---|---|---|---|---|
| `0359` | (PR #TBD lane1/scrum-2917) | SCRUM-2917 | `0359_scrum2917_materialize_run_id.sql` | RESERVED — file-only, pre-soak, T3, NOT applied; lands after 0358 (#1552) |
| `0360` | (PR #TBD lane1/scrum-2917) | SCRUM-2917 | `0360_scrum2917_secured_proof_predicate_hardening.sql` | RESERVED — file-only, pre-soak, T3, NOT applied; lands after 0358 (#1552) and 0359 |

## Recent migrations (SCRUM-2917) (PR #TBD lane1/scrum-2917)

- **0359_scrum2917_materialize_run_id.sql** (FILE-ONLY / PRE-SOAK / NOT APPLIED): SCRUM-2917 proof-materializer rollback marker — additive nullable `anchor_proofs.materialize_run_id uuid` stamped on rows the materializer INSERTs (existing rows stay NULL), plus partial index `idx_anchor_proofs_materialize_run_id ON (materialize_run_id) WHERE materialize_run_id IS NOT NULL`. Surgical rollback contract: `DELETE ... WHERE materialize_run_id = $1 AND merkle_root IS NULL AND proof_path IS NULL AND op_return_payload IS NULL` removes only that run's untouched skeletons — a row later reconstructed stops matching and is never deleted. Prefix 0359 per CTO ruling (Confluence 110198785) / SCRUM-2979. Tier T3. Rollback in the file header.
- **0360_scrum2917_secured_proof_predicate_hardening.sql** (FILE-ONLY / PRE-SOAK / NOT APPLIED): compensating `CREATE OR REPLACE` of `enforce_secured_anchor_proof_complete()` (0340 never edited, §1.2) applying the RULED completeness predicate when `arkova.proof_enforce_secured_complete='on'`: `(merkle_root IS NOT NULL AND proof_path IS NOT NULL) OR (proof_completeness_class = 'direct_anchored' AND op_return_payload IS NOT NULL)`. A bare `direct_anchored` label with NULL `op_return_payload` is rejected (§1.4 forge risk — a label is not proof); honest direct anchors keep merkle fields empty (never synthesize a degenerate branch). GUC gate, transition-into-SECURED logic, SECURITY DEFINER, `SET search_path = public`, and ERRCODE `check_violation` unchanged from 0340; only the predicate + exception message differ. Gated local-PG trigger test: `services/worker/src/jobs/proof-materializer-trigger.local.test.ts` (`PROOF_TRIGGER_PG=1`). Prefix 0360 per CTO ruling (Confluence 110198785) / SCRUM-2979. Tier T3. Full ROLLBACK (exact 0340 body) in the file header.
