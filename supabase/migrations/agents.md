...
| `0364` | `lane1/scrum-2905-security-advisor-revokes` / PR #1652 | SCRUM-2905 / 2918 | `0364_scrum2905_security_advisor_revokes.sql` — REVOKE anon/authenticated EXECUTE on `deduct_org_credit`/`deduct_credit` (keep service_role) | **PROD-APPLIED** 2026-07-27 ~13:26–13:32Z (part of the 0359–0364 batch; ledger reconciled to numeric 0364 per §0 rule 10); PR #1652 still open/DRAFT for the file-sync + soak-evidence record, exempted via #1712 |
| `0365` | `lane2/scrum-2940-folders` / PR #1657 | SCRUM-2940 | `0365_scrum2940_folders_table_and_anchor_link.sql` | **PROD-APPLIED** 2026-07-27 ~17:41–18:15Z. T3 (touches `supabase/migrations/` + ALTERs `anchors`). `folders` table + RLS/FORCE + nullable `anchors.folder_id` (metadata-only add) + owner-scope join trigger |
| `0366` | `lane2/scrum-2940-folders` / PR #1657 | SCRUM-2940 | `0366_scrum2940_anchors_folder_id_index.sql` | **PROD-APPLIED** 2026-07-27 ~17:41–18:15Z. **`CREATE INDEX CONCURRENTLY` — split out of 0365** because CONCURRENTLY cannot run in the migration builder's txn wrapper (lock-audit finding). Built successfully under live prod write load; `indisvalid=true` verified post-build |
| `0367+` (band) | advisor train (post-PI-0.5) | — | not yet filed | RESERVED band — advisor train does not inherit any PI-0.5 prefix; claim by adding a row here in the same PR |

Rows above predate the 2026-07-27 batch prod-apply; see HANDOFF.md "Migration ledger head is now `0366`" for the authoritative applied-state summary (`0358`–`0366` are now PROD-APPLIED; PRs remain open/Draft for file-sync + soak-evidence record-keeping). Full rig-reservation cross-reference: `docs/staging/rig-reservation-ledger-and-migration-registry-2026-07-20.md`.

## Recent migrations (PR SCRUM-2940 folders, #1657)

- **0365_scrum2940_folders_table_and_anchor_link.sql** + **0366_scrum2940_anchors_folder_id_index.sql** (PROD-APPLIED 2026-07-27 ~17:41–18:15Z): SCRUM-2940 folders. 0365 adds `public.folders` (owner_scope USER|ORG, exactly-one-owner CHECK, case-insensitive unique name per owner, RLS ENABLE + **FORCE ROW LEVEL SECURITY**, policies mirroring `anchors` owner split via `get_user_org_id()` / `is_current_user_platform_admin()`), a nullable `anchors.folder_id` FK (`ON DELETE SET NULL` = "delete folder → records fall back to Unfiled"), and `enforce_anchor_folder_owner_scope()` BEFORE trigger refusing cross-owner filing. The `folder_id` column add is metadata-only (no rewrite of the 2.97M anchors rows). 0366 is the `CREATE INDEX CONCURRENTLY idx_anchors_folder_id` **alone, no txn** — split out per the lock audit; applied under live production write load (`batch_insert_anchors` actively writing throughout) with zero write blocking. Tier T3. Rollback in each file header.
