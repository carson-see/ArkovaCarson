...
## Recent migrations (PR #NNNN — SCRUM-2485)

- **0355_scrum2485_public_anchor_base_projection_allowlist.sql**: `CREATE OR REPLACE get_public_anchor` — converts the base `'metadata'` sub-object from the `sanitize_metadata_for_public(...)` DENYLIST pass-through (strips named-PII + `_`-prefixed, passes EVERYTHING else) to an **EXPLICIT `jsonb_build_object` + `jsonb_strip_nulls` ALLOW-LIST** (mirrors the proven 0331 CPE/CLE pattern). Allow-list = the safe public display keys only: `title`, `credential_title`, `description`, `category`, `proof_url`, `issuer`, `jurisdiction` + the source-import evidence keys not already promoted to top-level (`evidence_schema_version`, `source_id`, `source_payload_content_type`, `source_payload_byte_length`, `extraction_method`, `extraction_manifest_hash`, `extraction_confidence`, `credential_id_hash`). A NEW anchors.metadata key (e.g. `registry.ctid`, `competencyFrameworks`) can no longer auto-project to anonymous callers (the public-by-default leak class). `sanitize_metadata_for_public` still wraps the free-form string values as defense-in-depth. `get_public_anchor_by_fingerprint` (0339) delegates to `get_public_anchor` and inherits the fix. Signature unchanged (`(text) → jsonb`) → no `database.types.ts` delta. SECURITY DEFINER + `search_path=public` + status filter + `deleted_at` guard preserved; `NOTIFY pgrst`. Recipient hash left byte-identical (SCRUM-2484 replaces it separately, same branch stack). §1.8 additive-nullable → no API version bump. Empirically validated on a throwaway local Postgres: leaky keys dropped, allow-listed keys kept. **T3** (migrations + security-sensitive anon projection); prod-apply RTE/Carson-gated post-soak — NOT applied by the authoring session. NOT merged — draft PR.

## Recent migrations (PR #NNNN — SCRUM-2484)

- **0356_scrum2484_public_anchor_recipient_hmac.sql**: `CREATE OR REPLACE get_public_anchor` — replaces the anon-projected `recipient_identifier` value (was a BARE UNSALTED `encode(extensions.digest(recipient::bytea,'sha256'),'hex')` since 0311 → offline-enumerable: anyone can `sha256(known_email)` and correlate) with a **KEYED HMAC-SHA256** over the `app.recipient_pepper` GUC (`extensions.hmac(lower(btrim(recipient))::bytea, pepper::bytea, 'sha256')`, same pattern as `validate_api_key` 0302/0303; GUC read like `app.base_url`). **FAILS CLOSED**: no pepper GUC ⇒ `recipient_identifier` returns `''` — never the enumerable bare sha256. The `recipient_identifier` KEY stays (frozen §1.8 API contract) — only its VALUE derivation changes → no version bump, no types delta. Preserves the SCRUM-2485 base-metadata allow-list (0355) verbatim + the full envelope (SECURITY DEFINER, search_path, status filter, deleted_at, NOTIFY). `get_public_anchor_by_fingerprint` (0339) delegates + inherits. Empirically validated on throwaway local Postgres: no-pepper→empty; pepper-set→keyed HMAC ≠ bare sha256, = expected `extensions.hmac`. **Carson/RTE-gated**: the pepper VALUE + `ALTER DATABASE ... SET app.recipient_pepper` + any `anchor_recipients` backfill are gated post-soak; until set, recipient_identifier reads '' (safe default). **T3**; prod-apply gated — NOT applied by the authoring session. NOT merged — draft PR.
