...
| `0364` | `lane1/scrum-2905-security-advisor-revokes` / PR #1652 | SCRUM-2905 / 2918 | `0364_scrum2905_security_advisor_revokes.sql` — REVOKE anon/authenticated EXECUTE on `deduct_org_credit`/`deduct_credit` (keep service_role) | RESERVED — pre-soak, file-only, NOT applied to prod |
| `0365+` | advisor train (post-PI-0.5); note `0365`/`0366` claimed by PR #1657 (SCRUM-2940 folders) on its own branch | — | not yet filed | RESERVED band — advisor train does not inherit any PI-0.5 prefix; claim by adding a row here in the same PR |
