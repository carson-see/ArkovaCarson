...
## In-flight migration reservations (0327–0335) — recorded 2026-06-01, updated 2026-06-05

`main` HEAD is **0331** as of 2026-06-05 (`0330`/`0331` merged ✓). The lowest unmerged prefix is `0327`. `0332`–`0334` are reserved (below); the next free prefix is **`0335`**. Canonical assignment for the currently-open / reserved migration PRs:
...
| `0335` | (Edge MCP Truthfulness PR-2) | BUG-1 | `0335_get_public_anchor_by_fingerprint.sql` | **reserved 2026-06-05** — verify-by-fingerprint DEFINER RPC; stacked on PR-1 (#1119); T3, soak pending |

- **Merge order must follow prefix order** (`0327→0335`): migrations apply monotonically, so merging a higher prefix before a lower one strands the lower one as out-of-order.
