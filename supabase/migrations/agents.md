...
| `0368` | `feat/scrum-2971-billing-idempotency` / PR (this) | SCRUM-2971 | `0368_scrum2971_billing_events_idempotency.sql` | RESERVED — pre-soak, file-only, NOT applied to prod/rig. T3 (touches `supabase/migrations/`, billing/money path). `NOT VALID` CHECK requiring `billing_events.idempotency_key IS NOT NULL OR stripe_event_id IS NOT NULL` on every new row (deliberately un-validated — legacy NULL rows exempted, no full-table scan/lock) so a client/network retry of a metered-usage call can no longer insert a duplicate audit row (=duplicate Stripe usage = overbilling risk). Paired with worker code deriving deterministic per-call idempotency keys at `billing/meteredBilling.ts` and `middleware/paymentTierRouter.ts` (ticket-named sites) plus `stripe/handlers.ts::recordBillingAudit` (found un-populated during the pre-migration insert-site audit, fixed in the same PR to avoid breaking the webhook audit-write path once the constraint lands) |
