...
| `0344` | `lane2/s1-reconciler-daily-sweep-caller` (PR #1260, Lane 2) | SCRUM-2349 | `0344_scrum2349_credit_conservation_invariant_fix.sql` | RESERVED — fixes the 0341 `org_credit_ledger_divergence` invariant (prod-apply-first, like 0343, before the migrations-vs-prod gate passes) |
...
- **0344_scrum2349_credit_conservation_invariant_fix.sql** (SCRUM-2349 / S1-9, T3): Fixes `org_credit_ledger_divergence` (added 0341). The 0341 body compared `balance` against a `p_initial_grant integer DEFAULT 0` scalar, but credit GRANTS are not in the `org_credit_deductions` ledger — they live in `org_credits.purchased` + `org_credits.monthly_allocation` plus net parent->child `org_credit_allocations`. With `p_initial_grant=0` every funded prod org false-flagged an error-level "Credit conservation VIOLATED" on the first reconciler tick (verified against live prod `vzwyaatejekddvltxyye`: 3 of 5 orgs funded). 0344 `DROP`s the `(uuid, integer)` signature and `CREATE OR REPLACE`s a `(uuid DEFAULT NULL)` one whose `expected = purchased + monthly_allocation + net(org_credit_allocations) + SUM(org_credit_deductions.amount)`, adding a `granted` column. `org_credit_allocations` EXISTS in prod (parent/child sub-org transfers) so the net-alloc term is included (received − given). Preserves `STABLE SECURITY DEFINER SET search_path = public`; `REVOKE ALL` from PUBLIC/anon/authenticated + `GRANT EXECUTE` to `service_role` only; `NOTIFY pgrst, 'reload schema'`. **ROLLBACK:** restore the 0341 `(uuid, integer)` body from `0341_scrum2349_2350_credit_integrity_foundation.sql`. **Prod-apply-first (like 0343):** the migrations-vs-prod ledger-drift gate requires the numeric prefix present in prod before merge; 0344 must be applied to prod (Carson-approved, via MCP `apply_migration` + §0 rule 10 numeric reconcile) ahead of the PR merging.
