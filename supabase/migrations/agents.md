...
- **0309_fix_anchors_rls_statement_timeout.sql** (renamed from 0307 to resolve prefix collision): Consolidated three separate `anchors` SELECT RLS policies into one with scalar subquery wrappers for InitPlan evaluation. Same pattern applied to `attestations` (5 branches including `status='ACTIVE'`).
