...
## Ratified 2-sprint plan migration reservations (2026-07-28)

Per the RTE migration assignment table in the CTO-ratified 2-sprint plan (`0367`–`0374` assigned; `0375`–`0378` spares, RTE-arbitrated). This row claims the first spare for L2-A5.

| `0375` | `feat/l2-a5-admin-credit-adjust` | L2-A5 (founder admin-controls: platform-admin org credit add/remove) | `0375_admin_org_credit_adjust.sql` | RESERVED — pre-soak, file-only, NOT applied to prod/rig. New RPC only (`admin_adjust_org_credit`) — REUSES the existing `org_credit_deductions` idempotency ledger (0326/0341, `entry_type` GRANT/REVOKE already provisioned) and `audit_events`; no new table |
...
## Recent migrations (L2-A5 admin credit adjust)

- **0375_admin_org_credit_adjust.sql** (FILE-ONLY / PRE-SOAK / NOT APPLIED — rides the 72h R17 soak per the ratified 2-sprint plan): adds `admin_adjust_org_credit(p_org_id, p_amount, p_reason, p_idempotency_key, p_actor)`, a `SECURITY DEFINER` RPC (service_role EXECUTE only) that lets a platform admin add/remove credits on `org_credits.balance`. **No new table** — reuses the existing `org_credit_deductions` idempotency ledger (0326, hardened append-only by 0341) via its already-provisioned `entry_type` values `GRANT`/`REVOKE`, and writes an `ORG_CREDIT_ADJUSTED` row to `audit_events` (actor + reason + before/after balance), all in one transaction. Balance never goes below zero (clean `insufficient_balance` error, not a raw `23514`). Idempotent on `(org_id, idempotency_key, reason)` retry. Money-conservation-invariant compatible with 0349's `org_credit_ledger_divergence` (it sums `org_credit_deductions.amount` unconditionally, so GRANT/REVOKE entries are picked up automatically — verified empirically, see PR). Verified against an isolated throwaway Postgres 15 container (not shared staging): 13 scenarios covering grant, revoke, idempotent replay, idempotency-key conflict, insufficient-balance rejection, invalid-amount/reason/key validation, auto-init of a missing `org_credits` row, append-only trigger still blocking direct mutation, and the money-conservation invariant holding exactly post-adjustment. Tier T3 (touches `supabase/migrations/`). Rollback in the file header.
