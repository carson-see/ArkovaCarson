...
- **0310_fix_anchors_rls_statement_timeout.sql** (renamed from 0309 to resolve prefix collision with 0309_expand_audit_event_category_constraint): Consolidated three separate `anchors` SELECT RLS policies into one with scalar subquery wrappers for InitPlan evaluation. Same pattern applied to `attestations` (5 branches including `status='ACTIVE'`).
