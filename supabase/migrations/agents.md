...
## Recent migrations (PR fix/f5-stats-fn-ownership)

| `0380` | `fix/f5-stats-fn-ownership` (this PR) | F-5 (`docs/staging/SOAK-FINDINGS-2026-08.md`) | `0380_f5_anchor_stats_fn_ownership_guard.sql` | **PROD-APPLIED 2026-08-01** (re-verified 2026-08-02 via Supabase MCP against `vzwyaatejekddvltxyye`: `pg_get_functiondef` shows the 42501 raise + `service_role` bypass on both functions, and `supabase_migrations.schema_migrations` carries numeric version `0380`). Applied ahead of merge because F-5 was a live, anon-callable, CTO-confirmed cross-tenant read. Numeric ledger head at claim time was `0378` (next-free stated as `0379`); this session was explicitly assigned `0380` to stay collision-proof against a concurrent sibling session in the same multi-agent F-1..F-8 soak-findings fix sweep also claiming a numeric prefix off the same `0378` head — `0379` was not independently re-verified free by this session, so `0380` was taken rather than risk a same-head collision. If `0379` lands first with a different fix, no renumber is needed; if `0379` is later found unclaimed, a future session may reuse it. |

- **0380_f5_anchor_stats_fn_ownership_guard.sql** (PROD-APPLIED 2026-08-01, ledger numeric `0380`; re-verified 2026-08-02 — see the table row above): F-5 — `get_org_anchor_stats(uuid)` / `get_user_anchor_stats(uuid)` were `SECURITY DEFINER` with a caller-supplied `p_org_id`/`p_user_id` and no ownership check (0378 explicitly deferred this exact pair — see 0378's header). Adds a body-level guard mirroring the existing `get_user_monthly_anchor_count` convention (`RAISE EXCEPTION ... USING ERRCODE = '42501'` on identity mismatch), gated on `get_user_org_id()` / `auth.uid()` respectively, with an explicit `get_caller_role() = 'service_role'` bypass for worker/admin callers. Grants unchanged — no REVOKE/GRANT in this migration; the guard alone closes the hole for every existing grantee. Confirmed via fresh grep this session: the only real caller is `src/pages/DashboardPage.tsx:213` via `src/lib/dashboardStats.ts`, which always passes the caller's own `profile.org_id` / `user.id` — dashboard unaffected. `services/worker/src/` has no call site for either function. Tier T3 (security-sensitive RPC body change). Rollback (exact pre-fix bodies) in file header.
  **Known non-leak edge:** the guard is `p_org_id IS DISTINCT FROM get_user_org_id()`, and for an anon caller both sides are NULL — `NULL IS DISTINCT FROM NULL` is FALSE — so a call passing an explicit NULL id skips the raise and returns `{total:0,secured:0,pending:0}` with HTTP 200 rather than a 403. No cross-tenant row is reachable that way (`WHERE org_id = NULL` never matches), so this is a response-shape wart, not a disclosure. Not patched in place: 0380 is already applied to prod and migrations are never edited after apply (§1.2) — closing it needs a compensating migration.
