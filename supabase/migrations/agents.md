...
## Recent migrations (PR #1260)

- **0349_scrum2349_credit_conservation_invariant_fix.sql** (SCRUM-2349 / S1-9, T3) — *renumbered 0344→0347 (2026-06-27) → 0349 (2026-06-28 base refresh): the prod ledger advanced to 0348 (`0347_lane1_i4_chain_block_hash_reorg`, `0348_scrum2353_webhook_event_claims`), so the prior 0347 slot collided; next free = max(prod head 0348)+1 = 0349*: Fixes `org_credit_ledger_divergence` (added 0341). The 0341 body compared `balance` against a `p_initial_grant integer DEFAULT 0` scalar, but credit GRANTS are not in the `org_credit_deductions` ledger — they live in `org_credits.purchased` + `org_credits.monthly_allocation` plus net parent->child `org_credit_allocations`. With `p_initial_grant=0` every funded prod org false-flagged an error-level "Credit conservation VIOLATED" on the first reconciler tick (verified against live prod `vzwyaatejekddvltxyye`, 2026-06-27: the live 0341 fn flags **3 of 5** orgs — balance 50/10/5, ledger empty, expected 0; the corrected expression flags **0 of 5**). 0349 `DROP`s the `(uuid, integer)` signature and `CREATE OR REPLACE`s a `(uuid DEFAULT NULL)` one whose `expected = purchased + monthly_allocation + net(org_credit_allocations) + SUM(org_credit_deductions.amount)`, adding a `granted` column. `org_credit_allocations` EXISTS in prod (parent/child sub-org transfers; 0 rows today so the net term is 0, but it is part of the true grant) so the net-alloc term is included (received − given). Preserves `STABLE SECURITY DEFINER SET search_path = public`; `REVOKE ALL` from PUBLIC/anon/authenticated + `GRANT EXECUTE` to `service_role` only; `NOTIFY pgrst, 'reload schema'`. **ROLLBACK:** in-file executable block restores the exact 0341 `(uuid, integer)` body. **Prod-apply-first (like 0343):** the migrations-vs-prod ledger-drift gate requires the numeric prefix present in prod before merge; 0349 must be applied to prod (Carson/RTE-gated, via MCP `apply_migration` + §0 rule 10 numeric reconcile) ahead of the PR merging.

## Recent migrations (PR #1427)

- **0354_proof_completeness_class_and_guc_reader.sql** (S3-A(2) / PROOF-BACKCATALOG, stacked on PR #1410, T3): adds `anchor_proofs.proof_completeness_class` — additive nullable text + `anchor_proofs_completeness_class_check` CHECK over EXACTLY the classifier's four class strings (`direct_anchored | batch_provable | already_complete | ambiguous`; NULL = unclassified) — and the `get_proof_enforcement_guc()` reader RPC (`STABLE SECURITY DEFINER SET search_path = public` over `current_setting('arkova.proof_enforce_secured_complete', true)`, REVOKE ALL from PUBLIC/anon/authenticated, GRANT EXECUTE to service_role only, `NOTIFY pgrst`). NO index added (per-anchor path served by `anchor_proofs_anchor_unique`; a class-partial index would only amplify the multi-million-row labeling writes — add later via operator `CREATE INDEX CONCURRENTLY` if a class-filtered read path materializes). The GUC stays OFF and the 0340 trigger is untouched — accepting `direct_anchored` as an alternative completeness proof is a separate, later trigger-amendment decision. Executable `-- ROLLBACK:` block in-file. Local `supabase db reset --local` green at head 0354; both `database.types.ts` regenerated. Prod-apply is Carson/RTE-gated (prod-apply-first for the migrations-vs-prod gate, like 0343/0349).
