...
## Recent migrations (PR #1618)

- **0362_scrum2913_public_anchor_registry_url_allowlist.sql** (**PROD-APPLIED 2026-07-27 ~13:26-13:32Z** via Supabase MCP, exempted via #1712 — see this PR's Staging Soak Evidence block for the full T3 soak + prod-apply record; §0-rule-10 ledger reconciled, numeric head 0364 verified post-apply): SCRUM-2913 Lane-2 deferred allow-list widening. `CREATE OR REPLACE get_public_anchor` based verbatim on the current prod-bound **0356** definition (0355 SCRUM-2485 base-metadata allow-list + 0356 SCRUM-2484 keyed-HMAC recipient), appending EXACTLY two keys to the base `'metadata'` allow-list: `registry_url` (CE registry **provenance** link, NOT a membership claim — R-7 §1.13) and `ce_envelope_sha256` (hex integrity fingerprint, §1.5 evidence model), both read `a.metadata ->> …` and additive-nullable via `jsonb_strip_nulls` (§1.8). Deferred from the Lane-3 CTDL importer **#1603** (`lane3/scrum-2913-ctdl-importer`), which maps `registryUrl→registry_url` / `ceEnvelopeSha256→ce_envelope_sha256`; key name `ce_envelope_sha256` is Lane-3's exact choice (avoids a `registry_envelope` claims-lint collision). **No schema change / no new column / no data migration** — the keys already live in `anchors.metadata`; this only widens what the anon-GRANTed projection is ALLOWED to surface. `Returns: Json` unchanged ⇒ no `database.types.ts` delta. SECURITY DEFINER + `search_path=public` + status filter + `deleted_at` guard + keyed-HMAC recipient preserved; `NOTIFY pgrst, 'reload schema'` included. Snapshot test `src/tests/scrum-2913-public-anchor-registry-url-allowlist.test.ts` asserts the exact widened key set (15+2) so accidental over-widening fails CI. Numbering: main head `0357`; open-PR band `0358`(#1552)/`0359`-`0360`(#1615 Lane 1)/`0360`(#1614)/`0361`(Lane 1 SCRUM-2916 watermark index) → **0362** is next free (advisor band shifts to 0363+). Tier **T3** (touches `supabase/migrations/` + security-sensitive anon projection). Rollback in the file header (restore the 0356 15-key allow-list + `NOTIFY pgrst`). **2026-07-28 (this PR, wave 0 conflict-resolve): the producer side was wired** — until now the allow-list was live in prod but NOTHING wrote `registry_url`/`ce_envelope_sha256` (an inert column). `services/worker/src/lib/credential-source-import.ts` now stamps both keys onto `anchors.metadata` for genuinely CE-Registry-sourced imports (host + real-CTID checks, anti-spoofing/anti-fabrication); `PublicVerification.tsx` / `SourceProvenanceDisplay.tsx` surface them. See `services/worker/src/lib/agents.md` and `src/components/verification/agents.md`.
