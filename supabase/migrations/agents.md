...
| `0345` | `fix/embed-public-records-timeout-0346` | SCRUM-2203 (0330 follow-up) | `0345_fix_embed_unembedded_query_perf.sql` | RESERVED — open PR (DRAFT, T3 soak pending). Next-free above main head `0344`; NOTE the branch name says `-0346` but the strictly-correct next-free prefix is `0345` (no open PR/reservation uses 0345) — file is `0345_*` to avoid a phantom gap. Fixes the embed-public-records cron statement-timeout (57014) regression: 0330's NOT EXISTS anti-join now walks ~2.1M already-embedded rows at the created_at leading edge. Adds `public_records.embedded_at` marker + maintenance trigger + partial index `idx_public_records_unembedded ((created_at) WHERE embedded_at IS NULL)` (standalone CONCURRENTLY + batched backfill, per 0313/0330 convention) + RPC rewrite to `embedded_at IS NULL`. |
