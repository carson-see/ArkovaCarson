...
| `0380` | `fix/f5-stats-fn-ownership` (this PR) | F-5 (`docs/staging/SOAK-FINDINGS-2026-08.md`) | `0380_f5_anchor_stats_fn_ownership_guard.sql` | RESERVED — pre-soak, file-only, NOT applied to prod/rig. Numeric ledger head at claim time was `0378` (next-free stated as `0379`); this session was explicitly assigned `0380` to stay collision-proof against a concurrent sibling session in the same multi-agent F-1..F-8 soak-findings fix sweep also claiming a numeric prefix off the same `0378` head — `0379` was not independently re-verified free by this session, so `0380` was taken rather than risk a same-head collision. If `0379` lands first with a different fix, no renumber is needed; if `0379` is later found unclaimed, a future session may reuse it. |

- **0380_f5_anchor_stats_fn_ownership_guard.sql** (FILE-ONLY / PRE-SOAK / NOT APPLIED): F-5 — `get_org_anchor_stats(uuid)` / `get_user_anchor_stats(uuid)` were `SECURITY DEFINER` with a caller-supplied `p_org_id`/`p_user_id` and no ownership check (0378 explicitly deferred this exact pair — see 0378's header). Adds a body-level guard mirroring the existing `get_user_monthly_anchor_count` convention (`RAISE EXCEPTION ... USING ERRCODE = '42501'` on identity mismatch), gated on `get_user_org_id()` / `auth.uid()` respectively, with an explicit `get_caller_role() = 'service_role'` bypass for worker/admin callers. Grants unchanged — no REVOKE/GRANT in this migration; the guard alone closes the hole for every existing grantee. Confirmed via fresh grep this session: the only real caller is `src/pages/DashboardPage.tsx:213` via `src/lib/dashboardStats.ts`, which always passes the caller's own `profile.org_id` / `user.id` — dashboard unaffected. `services/worker/src/` has no call site for either function. Tier T3 (security-sensitive RPC body change). Rollback (exact pre-fix bodies) in file header.
