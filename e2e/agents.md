...
| 2026-06-23 | WEBEXT-02/03/04 (SCRUM-2504/2505/2506): added `extraction-csp-fail-closed.spec.ts` — the §1.6 fail-closed exit proof. Runs the on-device privacy contract under the EXACT deployed CSP (parsed from `vercel.json`): asserts the CSP blocks the Tesseract/NER CDNs, `'self'` /vendor is reachable, and a model-load failure yields ZERO metadata egress. Self-contained (serves a probe page via `page.route` with the CSP header), unauthenticated. NOTE: like all specs here it needs `E2E_SUPABASE_SERVICE_KEY` at Playwright collection time (CI provides it); the production modules are also unit/integration-tested in `src/lib/`. |
