...
- External downloads (e.g. `tla2tools.jar`) MUST verify SHA256. See ci.yml's `Pin TLA2TOOLS_JAR` step for the canonical pattern (SCRUM-1248 / R0-2); when the upstream release binary changes, refresh the pin with a local `curl -fsSL <release-url> | shasum -a 256` check and update the inline date.
  - **`tla2tools.jar` re-pins are recurring, and they fail in the wrong direction.** Upstream `tlaplus/tlaplus` v1.8.0 is a **mutable pre-release tag** and has been re-cut four times (2026-07-09, 2026-07-18/21, 2026-07-31, 2026-08-11). Each re-cut turns the pin step red, and because `Run TLA+ PreCheck verification` is a separate step it is **SKIPPED** — so the model silently stops being verified while the failure reads like an infra hiccup. When you see `TLA+ Verification` fail, check *which step* failed before assuming the machine is broken.
  - Re-pin only after all **three** independent anchors agree: (1) the release body's SHA1 checksum table matches `shasum -a 1` of the downloaded jar, (2) the GitHub release API asset `digest` (`gh api repos/tlaplus/tlaplus/releases/tags/v1.8.0`), and (3) a direct `curl --proto '=https' | shasum -a 256`. Record all three plus asset `updated_at`/size in the inline comment, and move the old pin to the stale list. A mismatch across anchors is a supply-chain signal, not a re-pin.
  - Standing follow-up: vendor the jar or mirror it to an immutable location and pin to that, so upstream tag mutability can no longer disable TLA+ verification. No ticket filed yet.
