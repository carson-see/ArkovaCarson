...
function functionRef(fnName: string): string {
  return String.raw`(?:(?:"public"|public)\.)?(?:"${fnName}"|${fnName})`;
}

function hasInlineSearchPath(allSql: string, fnName: string): boolean {
  const pattern = new RegExp(
    String.raw`CREATE\s+OR\s+REPLACE\s+FUNCTION\s+${functionRef(fnName)}\s*\([^)]*\)[\s\S]*?AS\s*\$`,
    'gi',
  );
...
function hasAlterSearchPath(allSql: string, fnName: string): boolean {
  const pattern = new RegExp(
    String.raw`ALTER\s+FUNCTION\s+${functionRef(fnName)}\s*\([^)]*\)[\s\S]*?${SEARCH_PATH_RE.source}`,
    'i',
  );
