...
describe('SCRUM-1187 (AUDIT-06): payment_ledger view is SECURITY INVOKER', () => {
  ...
  it('payment_ledger view has security_invoker = true', () => {
    ...
    const alter = new RegExp(
      String.raw`ALTER\s+VIEW\s+(?:IF\s+EXISTS\s+)?${SCHEMA_PREFIX}${viewName}\s+SET\s*\([^)]*${SECURITY_INVOKER_KEY}\s*=\s*${SECURITY_INVOKER_TRUE_VALUE}[^)]*\)`,
      'i',
    );
    const createWith = new RegExp(
      String.raw`CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+${SCHEMA_PREFIX}${viewName}\s+WITH\s*\([^)]*${SECURITY_INVOKER_KEY}\s*=\s*${SECURITY_INVOKER_TRUE_VALUE}[^)]*\)`,
      'i',
    );
    expect(
