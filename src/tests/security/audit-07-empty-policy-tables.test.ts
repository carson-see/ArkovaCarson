...
const USER_ROLES = String.raw`(?:${ROLE}\s*,\s*${ROLE})`;

function tableRef(table: string): string {
  return String.raw`(?:(?:"public"|public)\.)?(?:"${table}"|${table})`;
}

function tableExists(allSql: string, table: string): boolean {
  return new RegExp(
    String.raw`CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?${tableRef(table)}\s*\(`,
    'i',
  ).test(allSql);
}
...
describe('SCRUM-1188 (AUDIT-07): explicit service-role-only RLS policies', () => {
  ...
  for (const tbl of SERVICE_ROLE_ONLY_TABLES) {
    it(`${tbl} has an explicit deny-all-users policy`, () => {
      ...
      const re = new RegExp(
        String.raw`CREATE\s+POLICY\s+${POLICY_NAME}\s+ON\s+${tableRef(tbl)}\s+(?:FOR\s+ALL\s+)?TO\s+${USER_ROLES}\s+USING\s*\(\s*false\s*\)\s+WITH\s+CHECK\s*\(\s*false\s*\)`,
        'i',
      );
  ...
  for (const tbl of SWITCHBOARD_TABLES) {
    it(`${tbl} has an explicit deny-write policy for non-service-role`, () => {
      ...
      const re = new RegExp(
        String.raw`CREATE\s+POLICY\s+${POLICY_NAME}\s+ON\s+${tableRef(tbl)}\s+FOR\s+(?:INSERT|UPDATE|DELETE|ALL)\s+TO\s+(?:${USER_ROLES}|${ROLE})[\s\S]*?(?:USING\s*\(\s*false\s*\)[\s\S]*?)?WITH\s+CHECK\s*\(\s*false\s*\)`,
        'i',
      );
