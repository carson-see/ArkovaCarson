...
- `nerPiiDetector.ts` — CLIENT-SIDE NER PII detection via transformers.js (`Xenova/bert-base-NER`, q8). **Self-hosted (SCRUM-2503):** loads weights from the Arkova app origin `/models/` (`public/models/`, covered by `connect-src 'self'`) with `env.allowRemoteModels = false` — it never hits the HF CDN. On a missing/failed load it throws a typed `NERModelLoadError` (never silently returns null), so the fail-closed stripper (`enhancedPiiStripper.ts`, Lane 2) can refuse to release text instead of falling back to regex-only (the old §1.6 fail-OPEN). Failed loads are NOT cached — a later call can retry. Vendor weights with `scripts/fetch-ner-model.ts` (ops step; `public/models/` is git-ignored, the binary is never committed).
