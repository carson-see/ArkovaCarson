...
Driving worker eslint warnings to zero so `--max-warnings 0` can be re-enabled (CLAUDE.md §0 Rule 9 / SCRUM-1250 R4). PRs #789, #791, #792 cover S1 work:
...
- **PR #792**: Security fix — 12 tenant isolation gaps (org_id on audit inserts, api_keys TOCTOU guard).
