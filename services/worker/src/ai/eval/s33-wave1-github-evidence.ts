...
/** Lane-3 consumer name; construction remains private to this module. */
export type S33AuthenticatedEvidenceBundle = AuthenticatedS33Wave1EvidenceBundle;
...
export function assertAuthenticatedS33Wave1EvidenceBundle(
  value: unknown,
): asserts value is Readonly<S33AuthenticatedEvidenceBundle> {
...
function authenticatedLane3AcceptanceFacts(evidence: Record<string, unknown>): {
  filename: typeof OUTPUT_FILENAMES.lane3Acceptance;
  artifactDigestSha256: string;
  rawSha256: string;
  canonicalSha256: string;
} {
  const lane3 = record(evidence.lane3Acceptance, 'Authenticated GitHub evidence lane3Acceptance');
  assertExactKeys(lane3, [
    'filename', 'artifactDigestSha256', 'rawSha256', 'canonicalSha256',
  ], 'Authenticated GitHub evidence lane3Acceptance');
  if (lane3.filename !== OUTPUT_FILENAMES.lane3Acceptance) {
    throw new Error('Authenticated GitHub evidence Lane-3 acceptance filename is invalid');
  }
  for (const key of ['artifactDigestSha256', 'rawSha256', 'canonicalSha256'] as const) {
    assertSha(lane3[key], SHA256_RE, `Authenticated GitHub evidence Lane-3 ${key}`);
  }
  return {
    filename: OUTPUT_FILENAMES.lane3Acceptance,
    artifactDigestSha256: lane3.artifactDigestSha256 as string,
    rawSha256: lane3.rawSha256 as string,
    canonicalSha256: lane3.canonicalSha256 as string,
  };
}
...
function renderEvidenceReport(evidence: Record<string, unknown>): string {
  ...
  const lines = [
    ...
    `- Lane-3 acceptance raw/canonical SHA-256: ${lane3.rawSha256} / ${lane3.canonicalSha256}`,
    '',
...
export async function authenticateS33Wave1GitHubEvidence(options: AuthenticateOptions): Promise<void> {
  ...
  const authenticatedBundle = registerAuthenticatedS33Wave1EvidenceBundle({
    ...
    trustedMainRepositoryRoot: mainRepositoryRoot,
  ...
  // CTO 102957057: the Team-3 consumer is loaded only after the private
  // WeakSet brand exists in this exact process. There is intentionally no
  // static import of the acceptance consumer in this trust-root module.
  const lane3AcceptanceModule = await import(LANE3_ACCEPTANCE_MODULE) as {
    createS33Wave1AcceptanceArtifactFromAuthenticatedEvidence: (
      evidence: Readonly<S33AuthenticatedEvidenceBundle>,
    ) => unknown;
  };
  const lane3Acceptance = lane3AcceptanceModule
    .createS33Wave1AcceptanceArtifactFromAuthenticatedEvidence(authenticatedBundle);
  const lane3AcceptanceRecord = record(lane3Acceptance, 'Lane-3 Wave-1 acceptance artifact');
  if (lane3AcceptanceRecord.schemaVersion !== 1
    || lane3AcceptanceRecord.artifactType !== 'arkova-s33-wave1-acceptance'
    || lane3AcceptanceRecord.batchId !== 'S33-W1'
    || lane3AcceptanceRecord.repositoryIdentity !== FIXED_REPOSITORY
    || lane3AcceptanceRecord.pullRequestNumber !== FIXED_PULL_REQUEST_NUMBER
  ...
  const artifactDigestSha256 = nonEmptyString(
    lane3AcceptanceRecord.artifactDigestSha256,
    'Lane-3 acceptance artifact digest',
  );
  assertSha(artifactDigestSha256, SHA256_RE, 'Lane-3 acceptance artifact digest');
  const { artifactDigestSha256: _claimedDigest, ...lane3AcceptanceWithoutDigest } = lane3AcceptanceRecord;
  if (sha256(canonicaliseJson(lane3AcceptanceWithoutDigest)) !== artifactDigestSha256) {
    throw new Error('Lane-3 acceptance artifact digest does not match its canonical facts');
  }
  const lane3AcceptanceDigests = writeCanonical(
