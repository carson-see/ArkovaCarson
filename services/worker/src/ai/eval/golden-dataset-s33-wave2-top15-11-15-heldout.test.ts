...
import { S33_WAVE2_TOP15_11_15_HELDOUT } from './golden-dataset-s33-wave2-top15-11-15-heldout.js';

const BATCH_ID = 'S33-W2-TOP15-11-15';
const REGISTRY_DIGEST_SHA256 = '89874d90dcab289164a022069efce85b0c62d82416be687d51829744c882e178';
const SOURCE_PATH = 'services/worker/src/ai/eval/golden-dataset-s33-wave2-top15-11-15-heldout.ts';
const MANIFEST_PATH = 'docs/lane4/s33-wave2-batches/top15-11-15/manifest.json';
const DATASHEET_PATH = 'docs/lane4/s33-wave2-batches/top15-11-15/datasheet.json';
const workerRoot = fileURLToPath(new URL('../../../', import.meta.url));
const repositoryRoot = fileURLToPath(new URL('../../../../../', import.meta.url));

const EXPECTED_TYPES = [
  ['legal-11-general-cle', 'CLE', ['general_cle']],
  ['legal-12-ethics-cle', 'CLE', ['ethics_cle']],
  ['legal-13-specialized-cle', 'CLE', ['specialized_cle']],
  ['legal-14-utility-patent', 'PATENT', ['utility']],
  ['legal-15-regulatory-instrument', 'REGULATION', ['federal', 'state']],
  ['financial-11-sec-10q', 'SEC_FILING', ['form_10q']],
  ['financial-12-sec-8k', 'SEC_FILING', ['form_8k']],
  ['financial-13-sec-def14a', 'SEC_FILING', ['form_def14a']],
  ['financial-14-finra-broker', 'FINANCIAL_ADVISOR', ['finra_registered']],
  ['financial-15-investment-adviser', 'FINANCIAL_ADVISOR', ['sec_registered', 'state_registered']],
  ['education-11-trade-certification', 'CERTIFICATE', ['trade_certification']],
  ['education-12-training-certificate', 'CERTIFICATE', ['training_certificate']],
  ['education-13-completion-certificate', 'CERTIFICATE', ['completion_certificate']],
  ['education-14-accreditation', 'ACCREDITATION', ['institutional', 'programmatic']],
  ['education-15-microcredential', 'BADGE', ['educational_microcredential']],
] as const;
const EXPECTED_TYPE_ORDER = EXPECTED_TYPES.map(([typeId]) => typeId);
...
describe('S3.3 Wave 3 top-15 tranche 11-15', () => {
  ...
  it('is one immutable producer-only 180-row, 15-type whole batch', () => {
    ...
    expect(manifest.source).toMatchObject({ path: SOURCE_PATH, exportName: 'S33_WAVE2_TOP15_11_15_HELDOUT' });
    ...
    expect(S33_WAVE2_TOP15_11_15_HELDOUT).toHaveLength(180);
  });

  it('binds source and datasheet blobs and preserves exact frozen 11-15 order/mappings', () => {
    ...
    for (const [typeId, credentialType, subTypes] of EXPECTED_TYPES) {
      const rows = manifest.entries.filter(({ registryTypeId }) => registryTypeId === typeId);
      expect(rows, typeId).toHaveLength(12);
      expect(rows.every((row) => row.credentialType === credentialType), typeId).toBe(true);
      const dataRows = rows.map(({ id }) => datasheet.rows.find((candidate) => candidate.id === id)!);
      expect(dataRows.filter(({ edgeCase }) => edgeCase), typeId).toHaveLength(4);
      expect(dataRows.every(({ subType }) => subTypes.includes(subType as never)), typeId).toBe(true);
    }
  ...
  it('is an exact ordered bijection disjoint from both predecessor batches', () => {
    const prior = [...S33_WAVE2_TOP15_01_05_HELDOUT, ...S33_WAVE2_TOP15_06_10_HELDOUT];
    const priorIds = new Set(prior.map(({ id }) => id));
    const priorFingerprints = new Set(prior.map(({ strippedText }) => sha256(normalizeForFingerprint(strippedText))));
    const ids = new Set<string>();
    const fingerprints = new Set<string>();
    for (const [index, entry] of S33_WAVE2_TOP15_11_15_HELDOUT.entries()) {
  ...
  it('keeps each row deep, concrete, held out, PII-free, and provenance complete', () => {
    const edgeClasses = new Set(S33_EDGE_CLASSES);
    for (const [index, entry] of S33_WAVE2_TOP15_11_15_HELDOUT.entries()) {
      const row = datasheet.rows[index];
      expect(entry.provenance).toBe('authored-s33-lane4');
      expect(entry.source.startsWith('authored/s33-wave2/top15-11-15/')).toBe(true);
  ...
  it('has zero full-text/id and exact normalized 6-13-gram leakage', () => {
    ...
    expect(checkHeldoutLeakage(S33_WAVE2_TOP15_11_15_HELDOUT, corpus)).toEqual([]);
    const prior = [...S33_WAVE2_TOP15_01_05_HELDOUT, ...S33_WAVE2_TOP15_06_10_HELDOUT];
    const comparisons = [
      ...corpus.map(({ path, content }) => ({ id: path, text: content })),
      ...prior.map(({ id, strippedText }) => ({ id, text: strippedText })),
    ];
    expect(exactNgramHits(
      S33_WAVE2_TOP15_11_15_HELDOUT.map(({ id, strippedText }) => ({ id, text: strippedText })),
      comparisons,
    )).toEqual([]);
  }, 30_000);
});
