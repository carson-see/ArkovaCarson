...
- SCRUM-2210 (`billing.ts`): added `GET /api/billing/status` → `handleBillingStatus`. This is the `BillingInfo` endpoint the frontend `BillingPage` has always fetched but that was never implemented (`billingRouter` only had `/checkout/session` + `/billing/portal`) → 404 → billing page bricked. **Always returns 200** with a usable `BillingInfo`: a free-tier default when the caller has no subscription, and a best-effort usage count (`recordsUsed` falls back to 0 if the `anchors` count errors/times out) so a downstream failure can't brick billing (the SCRUM-1983 / SCRUM-2213 lesson). Read-only; uses `rateLimiters.api` (60/min).
