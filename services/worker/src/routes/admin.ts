...
adminRouter.get('/queue/pending', async (req, res) => {
  ...
  if (!userId) { res.status(401).json({ error: 'Authentication required' }); return; }
  try {
    await handleListPendingResolution(userId, req, res);
  } catch (error) {
...
adminRouter.post('/rules/test', async (req, res) => {
  const userId = await extractAuthUserId(req);
  if (!userId) { res.status(401).json({ error: 'Authentication required' }); return; }
  try {
    await handleTestRule(userId, req, res);
  } catch (error) {
    logger.error({ error }, 'Rules test request failed');
    res.status(500).json({ error: 'Internal server error' });
  }
});

adminRouter.get('/rules/:id/executions', async (req, res) => {
  const userId = await extractAuthUserId(req);
  if (!userId) { res.status(401).json({ error: 'Authentication required' }); return; }
  try {
