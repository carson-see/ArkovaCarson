...
- `orgCredits.ts` — `deductOrgCredit()` wraps the `deduct_org_credit` RPC. Returns `{allowed, error?, balance?, required?, idempotent?}`; SCRUM-1649 FAST_TRACK retries use `idempotent=true` to detect a reused credit deduction. **SCRUM-2350 (QUEUE-04)** adds `debitAndEnqueueAnchor(db, {orgId, anchorId, reason, amount?, targetStatus?, expectedStatus?})` wrapping the atomic `debit_and_enqueue_anchor` RPC (migration 0341) — debits one org credit AND transitions the anchor status in ONE transaction (reference_id = per-anchor id, NOT batch id). Returns `{allowed, error?, balance?, required?, idempotent?, anchorStatus?}`; `insufficient_credits`/`anchor_not_in_expected_status` mean no debit happened. Test: `debitAndEnqueueAnchor.test.ts`. The migration also makes `deduct_org_credit` require a non-null `reference_id` and store signed amounts, and converts `refund_org_credit` to an append-only positive `REFUND` row (never DELETE).
