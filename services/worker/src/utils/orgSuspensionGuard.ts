 ...
 *
 * SCRUM-1647 follow-up (CodeRabbit Major): a `db.rpc()` rejection used to
 * bubble out as an unhandled promise, and `{ data: null, error: null }` was
 * treated as ok=true, which would let a write proceed against a half-broken
 * DB. Both are now treated as guard_lookup_failed.
 */
export async function ensureOrgNotSuspended(orgId: string): Promise<OrgSuspensionGuardResult> {
  ...
  // null/undefined/non-boolean — treat as lookup anomaly per CodeRabbit
  // fail-closed guidance. is_org_suspended()'s SQL is `coalesce(..., false)`
  // so the RPC itself never returns null; this only fires if the pooler /
  // PostgREST returned a malformed payload.
