...
- **2026-07-08 #1408 — transient-vs-definitive fault classification on the header/proof fetch.** `fetchConfirmationProof` previously mapped EVERY thrown error from the `Promise.all([getBlockHeaderHex, getTxOutProof])` fetch to `stale`, so a TRANSIENT RPC failure (5xx/429/timeout/network drop — the provider already exhausted its own `retryWithBackoff`) on a genuinely-confirmed tx was poisoned to `stale` instead of retried. It now branches on `isRetryableError(err)`: transient → `pending` (reason `…failed transiently — will retry`, re-fetched next cron tick), definitive (RPC application error like "block not found", 4xx, any non-retryable) → `stale` (reason `…(non-retryable)`). **`utxo-provider.ts` `isRetryableError`** now also treats HTTP **429** as retryable (`status >= 500 || status === 429`) — a 429 is a throttle, not a hard 4xx (§1.10 Retry-After); this also stops a 429 while fetching a confirmation proof from being mis-classed definitive. This classification is the CHANGED surface driven by the targeted soak `services/worker/src/jobs/confirmation-proof-fault-driver.ts` (unit, red-first) + `scripts/staging/confirmation-proof-fault-harness.ts` (rig). T3.
