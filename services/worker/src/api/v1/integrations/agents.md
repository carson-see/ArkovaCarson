...
**Live-window honesty:** the org-UUID legacy-token acceptance path stays live in prod AFTER this PR merges until BOTH (a) an operator creates the `drive-subscription-renewal` Cloud Scheduler job (it is declared in `scripts/gcp-setup/cloud-scheduler.sh` but not yet applied as of PR #1944) and (b) each existing connection actually passes through a renewal cycle — up to ~7 days after the job starts running, bounded by each connection's current channel expiry. Merging this PR does not itself close the vulnerability window; the operator step does.
