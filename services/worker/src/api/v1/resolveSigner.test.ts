/**
 * Tests for `resolveSigner()` in verify-proof.ts (SCRUM-900 close-out).
 *
...
describe('resolveSigner — env-driven resolution order (SCRUM-900)', () => {
  ...
  it('KMS takes precedence over PEM when both are set (production-priority rule)', () => {
    ...
    process.env.PROOF_SIGNING_KEY_ID = KEY_ID;
    // The KMS branch returns a closure that defers KMS-client init until
    // the first sign() call. The PEM branch eagerly parses the PEM and
    // would throw if invoked. Because the KMS branch wins, resolveSigner
    // returns cleanly and the malformed PEM is never touched.
    const signer = resolveSigner();
    expect(signer).not.toBeNull();
    expect(signer?.keyId).toBe(KEY_ID);
  });
