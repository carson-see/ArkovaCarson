...
async function dispatchFastTrackAnchor(rule: RuleRow, exec: ExecutionRow): Promise<Outcome> {
  ...
  // anchor job. Goes through the shared `deductOrgCredit` helper (SCRUM-1170-B)
  // so `config.enableOrgCreditEnforcement=false` short-circuits cleanly in
  ...
  if (result.allowed) {
    ...
    if (!jobId) {
      ...
      // job. A transient outcome would re-call deduct_org_credit on the next
      // pass and double-charge the org until migration 0278's RPC gains
      // idempotency on p_reference_id (SCRUM-1170-B). Mark permanent so
      // execution lands in FAILED, not RETRYING.
      logger.error(
