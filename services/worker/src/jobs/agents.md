...
- **Queue-run credit gate parity** (SCRUM-1649): The legacy batch-anchor fallback path must enforce the same org credit gate as the claim-RPC path before broadcasting anchors. Queue credit metadata writes are Zod-validated before updating `anchors.metadata`; if validation or credit marking fails, the anchor is released to `PENDING` and is not broadcast.
- **Atomic debit+enqueue** (SCRUM-2350 / QUEUE-04): `applyQueueRunCreditGate` now charges the per-anchor queue-run credit via the atomic `debit_and_enqueue_anchor` RPC (worker wrapper `debitAndEnqueueAnchor`, migration 0341) — the debit row and the anchor status commit (or roll back) together, closing the old debit→separate-mark window. The charge is keyed by the per-anchor id, idempotent on retry, and uses "confirm-and-debit" (expected==target = the anchor's already-claimed status). Because the ledger is now append-only + idempotent, a post-debit metadata-mark failure no longer triggers a compensating refund — the anchor is released with a denial marker and the reconciler (`org_credit_ledger_divergence`) surfaces any stranded debit. The post-**broadcast** chain-failure compensation still calls `refund_org_credit` (now an idempotent, append-only positive `REFUND` row), so a failed refund there is recoverable rather than an irreversible double-charge; the `DOUBLE_BILLING_RISK` log on that path is retained as a fail-closed signal.
