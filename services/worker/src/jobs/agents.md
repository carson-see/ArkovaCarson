...
**Reused, not reimplemented — `dispatchFastTrackAnchor`'s credit-safety core is now `dispatchCreditFundedAnchor(rule, exec, options)`, shared verbatim by both actions.** The manual "Secure Instantly" UI control (`SecureDocumentDialog.tsx` / `useSecuringCapability.ts`, SCRUM-2894) was traced end to end first, per the task brief — it turned out to be a dark, hardcoded-`canSecureInstantly:false` placeholder (CTO ruling R5) whose `handleConfirm(..., 'instant')` branch is a bare client-side `supabase.from('anchors').insert()` with no server call and no real credit consumption to reuse. The actual credit-funded, idempotent, retry-safe "secure now" implementation that exists in this codebase is `FAST_TRACK_ANCHOR`'s (SCRUM-1649 DS-06) — so `dispatchInstantSecure` shares that implementation via the new `dispatchCreditFundedAnchor` helper rather than duplicating ~115 lines of credit-safety-critical logic. `dispatchFastTrackAnchor`'s own observable behavior, reason strings, and full pre-existing test suite (29 cases) are unchanged by the refactor — confirmed by running them post-refactor, not just eyeballing the diff.

- **Idempotency — identical mechanism to FAST_TRACK_ANCHOR, by construction.** `deductOrgCredit(db, rule.org_id, 1, 'rule.instant_secure', exec.id)` — the reference key is `exec.id` (the execution row's OWN id), never `attempt_count`. A dispatcher retry re-runs the same execution row and passes the same `exec.id`, so `deduct_org_credit`'s `UNIQUE(org_id, reference_id, reason)` ledger (migration `0326`) either no-ops as a detected replay (`idempotent:true, deducted:0`) or refuses on a genuine amount mismatch — it can never double-charge. Verified with an explicit dispatcher-level retry test asserting `p_reference_id: EXEC_ID` on both the original call and the retry.
- **Insufficient credits → fall back to the free queue, never fail or drop the document.** Same `insertAnchorQueueItem(withCreditDenialReason(...))` fallback FAST_TRACK_ANCHOR already uses (`credit_denial_reason: 'insufficient_credits'`, `outcome: queued_for_anchor`, `routed_to: anchor_queue`) — the promise "queued, not anchored" is kept. **New, INSTANT_SECURE-only requirement** (the founder directive's explicit "so someone notices the fallback happened," which FAST_TRACK_ANCHOR's existing fallback does NOT do): `notifyInstantSecureCreditDenial()` logs a `logger.warn` and calls `emitOrgAdminNotifications({ type: 'rule_fired', ... })` with `reason: 'insufficient_credits'` in the payload, itself wrapped so a notification-delivery failure can never turn an already-correctly-queued anchor into a retried/failed execution.
- **Downstream failure after a successful charge (anchor insert fails, or the `anchor.fast_track` job enqueue fails) → refund and retry.** `compensateCreditFundedAnchorFailure` (renamed/parameterized from `compensateFastTrackCreditFailure`) calls `refund_org_credit` with a reason distinct per action (`rule.instant_secure_compensation` vs `rule.fast_track_anchor_compensation`) so the two actions' ledger rows stay independently auditable, then marks the execution `RETRYING` (or `FAILED` if the refund itself fails — never silently eats a charged-but-unrecoverable credit).
- **What makes it actually "instant," not just relabeled:** on the happy path, after the shared credit+insert+job-submit steps succeed, `dispatchInstantSecure` additionally calls `processBatchAnchors({ force: true, orgId: rule.org_id })` — the SAME primitive `org-queue-scheduler.ts` (SCRUM-1130) already uses in prod to force-drain one org's queue out of band from the standard triggers, including its existing cross-instance run-lease safety (a concurrent call elsewhere simply doesn't acquire the lease and no-ops rather than racing). This call is **best-effort only** (`triggerImmediateAnchorPass`, wrapped in try/catch, logs and swallows on failure) — the anchor is already safely PENDING and will be picked up by the standard triggers regardless, so an outage in the accelerator must never look like a lost document or justify a retry. **Not** called on the insufficient-credit fallback path (nothing to accelerate — that anchor is a plain free-queue item like any other) or the SCRUM-2904 connector-deferral path (no anchor was even materialized).
- **`batch-anchor.ts`'s `queueRunCreditReason()` gained a parallel branch** for INSTANT_SECURE, mirroring the existing FAST_TRACK_ANCHOR one: an INSTANT_SECURE anchor that fell back to the queue at dispatch time (`credit_denial_reason: 'insufficient_credits'`) gets a second charge attempt (`rule.instant_secure_queue_run`) when the batch eventually claims it — if the org topped up credits in the meantime, charge once here instead of anchoring it for free forever. An already-charged INSTANT_SECURE anchor (`credit_denial_reason: null`) correctly falls through to no second charge, same as FAST_TRACK_ANCHOR.
- **Found in passing, NOT fixed here (flagged separately):** `FAST_TRACK_ANCHOR`'s `anchor.fast_track` job — enqueued via `submitJob()` on every successful dispatch — has **zero consumer anywhere in this codebase**. `claimJob('anchor.fast_track')` is never called (grep-verified across all of `services/worker/src`; the only `claimJob()` call site anywhere is `professional-education-extraction.ts`, a wholly unrelated job type). Confirmed empirically: prod `job_queue` carries **zero rows** of type `anchor.fast_track` (queried 2026-08-03). This means FAST_TRACK_ANCHOR, despite its name, has never actually anchored any faster than the free queue path — it just charges a credit for the same wait. INSTANT_SECURE does NOT inherit this gap (its actual speed comes from the `processBatchAnchors` call above, not the job enqueue, which it still does for job-tracking/idempotency-reuse parity with FAST_TRACK_ANCHOR). Fixing FAST_TRACK_ANCHOR's dead consumer (wire a `/jobs/anchor-fast-track` cron draining `anchor.fast_track` via the same `processBatchAnchors({force,orgId})` pattern, register it in `scripts/gcp-setup/cloud-scheduler.sh` + `scheduler-manifest.ts`) is real, scoped, low-risk work — left for its own PR rather than riding in on this one, since it touches a different action type's established behavior.
- Tests: `rule-action-dispatcher.test.ts` `describe('INSTANT_SECURE (founder directive 2026-08-03)')` — 10 cases (happy path, retry-no-double-charge, insufficient-credit fallback + log/notify, a throwing notification does not fail the dispatch, credit-RPC-throw, org_not_initialized, post-charge refund-and-retry on both anchor-insert and job-submit failure, best-effort-batch-trigger-failure-does-not-fail-the-dispatch, SCRUM-2904 connector-deferral). `batch-anchor.test.ts` gained 3 cases for the `queueRunCreditReason` branch (second-charge-attempt success, second-attempt-also-denied stays PENDING, already-charged anchor not double-charged). Full pre-existing `rule-action-dispatcher.test.ts` (29) and `batch-anchor.test.ts` (39) suites re-run green post-refactor (39 and 42 total respectively, including the new cases).

**Dead-rule bug — investigated, NOT reproduced; see `services/worker/src/rules/agents.md` for the evaluator-level fix (a durable characterization test, not a behavior change) and `services/worker/src/api/agents.md` for a related dashboard-metric bug found in passing** (`compliance-inbox-summary.ts`'s `secured_automatically` counter querying a `routed_to` value the dispatcher has never emitted since SCRUM-1649 DS-AUTO-02 — silently zero for every org, which independently explains a chunk of the "doesn't look like it's working" perception).
...
- **`IntentReconcileResult` and `TxidJournalReconcileResult` both gain an optional `rejectedReason?: string`.** `reconcileTxidJournals`'s REVERT branch sets it to `` `Txid journal REVERTED after affirmative absence (${decision.reason})` `` — reusing the reason the branch already resolves with, never inventing new copy. `reconcileOneIntent`'s reject-and-unwind branch sets it to `errMessage(err)` — the exact error already passed to the `'Intent reconcile: broadcast definitively rejected — unwinding intent'` log line just above it.
- **`reconcileBroadcastIntents` seeds its result from the journal pass, then lets the legacy-compat loop overwrite it** if that ALSO finds a reject in the same pass: a direct node-level refusal (`isBroadcastRejectedError`) is more specific and more recently resolved than an affirmative-absence-after-timeout REVERT, so it wins if both occur.
- **`_processBatchAnchorsInner` captures `reconcile.rejectedReason` into a local `reconcileRejectedReason`, then routes every post-reconcile "nothing else to do" early return through a new `emptyResult()` helper** (`{ ...EMPTY, rejectedReason: reconcileRejectedReason }` when set) instead of the bare `EMPTY` const. This covers: treasury-empty, no-PENDING-anchors, trigger-not-met, fee-ceiling-exceeded, claim-RPC-timeout/failure, and below-`MIN_BATCH_SIZE`-after-credit-gate. The ONE pre-reconcile early return (invalid `orgId`, before `reconcileBroadcastIntents` even runs) is untouched — nothing to merge yet at that point.
...
## 2026-08-02 — merge resolution note (PR #1764 rebased onto #1839/#1845)

PR #1764 (silent credit/billing RPC alerting) is additive — it wires `captureCreditRpcFailureAlert()`
into existing failure branches and does not touch id-filter chunking. It predates #1839/#1845 and
conflicted only on import lines and one `makeMock()` helper signature in
`publicRecordAnchor.test.ts` (both PRs added a parameter/mock to the same function). Both sides kept:
main's `chunkForInFilter` import alongside this PR's `captureCreditRpcFailureAlert` import in
`batch-anchor.ts`; `publicRecordAnchor.ts` merged clean (no overlapping lines — the sentry alert call
sites and the chunking call sites are disjoint). `publicRecordAnchor.test.ts`'s `makeMock()` now
takes both the hoisted `captureCreditRpcFailureAlert` mock (this PR) and the
`{ revertError, claimError, fetchError }` options param (main, #1812/#1839). Full `src/jobs/` +
`src/api/v1/` + `src/middleware/` + `sentry.test.ts` suite green post-merge (235 files, 3217 tests).
...
- **`platform-health-digest.ts` / `platform-health-digest-cron.ts`** (new, `feat/platform-admin-daily-health-digest`, draft T2, 2026-08-18) — daily platform-state summary email to platform admins. Same two-file split as `queue-digest`: `platform-health-digest.ts` is the **pure engine** (`buildPlatformHealthPayload()` + `deliverPlatformHealthDigestToAdmin()`, reuses `queue-digest.ts`'s `assertNoRawContent` guard directly rather than duplicating it); `platform-health-digest-cron.ts` is the **production wiring** (`listPlatformAdmins()` reads `profiles.is_platform_admin=true` — never a hardcoded address; `assemblePlatformHealthSnapshot()` reads anchors-by-status, `job_queue` depth, `anchor_txid_journal` for the last-24h batch flush, `connector_alert_state` for a platform-wide connector rollup, and `org_credits`/`anchors` for test-tier quota anomalies). Unlike the queue digest, content is **one snapshot fanned out identically to every recipient**, not per-admin scoped, and there is no suppression/opt-out (platform admins are staff, not a customer preference surface) — only idempotency/retry, same F3 reserve-before-send pattern, backed by NEW audit_events event types `PLATFORM_HEALTH_DIGEST_SENT`/`_FAILED` (no migration). Every section reader degrades to `null` ("not measured") on its own read error rather than sinking the whole digest or asserting a false zero — see `assemblePlatformHealthSnapshot`'s `Promise.all` + per-section `safe()` wrapper. Gated by `ENABLE_PLATFORM_HEALTH_DIGEST` (default **true**, unlike queue-digest's default-false — this is a routine internal digest, not a new customer-facing send). Prod trigger: `POST /jobs/platform-health-digest`; NOT yet bound to Cloud Scheduler (see `scripts/gcp-setup/agents.md`'s dated entry). Keeps the existing hardcoded-recipient stuck-anchor alert in `pipeline-health.ts` completely separate, on purpose — different signal shape (threshold alarm vs. routine digest).
...
- Silent credit-RPC pre-mortem — `credit-expiry.ts` (`allocate_monthly_credits`), `monthly-allocation-rollover.ts` (`roll_over_monthly_allocation`, per-org), `publicRecordAnchor.ts` (`batch_insert_anchors`), `batch-anchor.ts` (`submit_batch_anchors`, 3 sites: intent-reconcile defer, retry-exhausted fallback, legacy-path fallback) now call `captureCreditRpcFailureAlert()` (`utils/sentry.ts`) on their RPC-error branches — previously only `logger.error`, no Sentry alert, so a failure could go undetected for a full billing cycle. Fail-mode tagging (`open`/`closed`/`retried`) is honest per call site; behavior is UNCHANGED, only observability was added. See `utils/agents.md` for the shared helper contract.
