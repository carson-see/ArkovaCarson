...
- Cron route: `POST /jobs/drive-subscription-renewal` in `routes/cron.ts`, hourly. In-process dev/test backup (unconditional, no feature flag) in `routes/scheduled.ts`. Cloud Scheduler declaration in `scripts/gcp-setup/cloud-scheduler.sh` — **not yet applied to prod as of this PR**; running that script against prod is an operator step (see the PR body).
