...
- **`credit-conservation-reconciler.ts` (S1-9 / SCRUM-2349 / PM-25)** — money-conservation reconciler daily-sweep CALLER. The reconciler LOGIC is the prod SQL function `org_credit_ledger_divergence(p_org_id uuid DEFAULT NULL, p_initial_grant integer DEFAULT 0)` (live via mig 0341; service_role EXECUTE, STABLE SECURITY DEFINER), which checks `balance == initial_grant + SUM(org_credit_deductions.amount)` per org (the ledger is append-only, signed: DEBIT<0/REFUND>0). This module is the net-new caller. `decideCreditConservationAlert(rows)` is a pure decision fn; `runCreditConservationReconciler(db)` is the cron glue: it fires the RPC over ALL orgs (`p_org_id`=NULL via `callRpc`), filters `diverged=true`, builds a conservation report, and on any drift logs at error + calls `captureCreditConservationAlert()` (stable `credit-conservation-reconciler` Sentry fingerprint so daily re-fires collapse to one issue) — the gate #11 SLO/alerting signal. **Read-only** reconciliation → idempotent/safe to re-run, and (unlike the anchor jobs) NOT in `ANCHOR_TABLE_IN_PROCESS_JOBS`: a paused anchor pipeline under the maintenance flag must NOT silence credit-ledger integrity checks (it touches credit ledger tables, not `anchors`). **PII (§1.4):** logs + Sentry carry `org_id` + divergence MAGNITUDE only — never raw `balance`/`ledger_sum`/`expected` (raw credit amounts are PII); per-org alert rows capped at 50. Probe failure (RPC error/throw) → `healthy:false` with `divergedCount:null` so a broken read never masquerades as a clean report. Cron route `/jobs/reconcile-credit-conservation` (detected drift → 200, no Scheduler retry; probe failure → 500). In-process backup in `scheduled.ts` (daily `0 9 * * *`). Pure fn + cron-glue shape mirrors `stuck-anchor-monitor.ts`. **Scheduler/deploy binding is T3 + serialized under Carson** — this PR builds + tests only.
