...
- Cron route: `POST /jobs/drive-subscription-renewal` in `routes/cron.ts`, hourly. **No `scheduled.ts` in-process backup** — deliberately, matching `docusign-reconciliation`'s precedent (Cloud-Scheduler-only): an unconditional in-process schedule at the same cadence as the Cloud Scheduler job would double-fire outside CPU-throttled windows, racing two sweeps against the same due connections and wasting a channel registration on whichever one loses. Cloud Scheduler declaration in `scripts/gcp-setup/cloud-scheduler.sh` — **not yet applied to prod as of this PR**; running that script against prod is an operator step (see the PR body). Until that job exists in Cloud Scheduler, this route ONLY fires on a manual `curl`/operator trigger — no renewal happens automatically at all.
