...
- **`credit-conservation-reconciler.ts` (S1-9 / SCRUM-2349 / PM-25)** — money-conservation reconciler daily-sweep CALLER. The reconciler LOGIC is the prod SQL function `org_credit_ledger_divergence(p_org_id uuid DEFAULT NULL)` (added by mig 0341, **CORRECTED by mig 0347** (originally 0344; renumbered on the 2026-06-27 base refresh); service_role EXECUTE, STABLE SECURITY DEFINER), which checks `balance == purchased + monthly_allocation + net(org_credit_allocations) + SUM(org_credit_deductions.amount)` per org. **0347 bug fix:** the original 0341 body compared against a `p_initial_grant integer DEFAULT 0` scalar, but credit GRANTS are NOT in the deduction ledger — they live in the `org_credits` columns (`purchased` + `monthly_allocation`) plus net parent→child `org_credit_allocations`. With `p_initial_grant=0` every funded org false-flagged a VIOLATION on the first tick (confirmed against live prod `vzwyaatejekddvltxyye`); 0347 drops the scalar arg, sources the grant from the real columns, and adds a `granted` column. This module is the net-new caller. `decideCreditConservationAlert(rows)` is a pure decision fn; `runCreditConservationReconciler(db)` is the cron glue: it fires the RPC over ALL orgs (arg-less → `p_org_id` keeps SQL DEFAULT NULL via `callRpc`; **no `p_initial_grant` arg** — its presence was the bug), filters `diverged=true`, builds a conservation report, and on any drift logs at error + calls `captureCreditConservationAlert()` (stable `credit-conservation-reconciler` Sentry fingerprint so daily re-fires collapse to one issue) — the gate #11 SLO/alerting signal. **Read-only** reconciliation → idempotent/safe to re-run, and (unlike the anchor jobs) NOT in `ANCHOR_TABLE_IN_PROCESS_JOBS`: a paused anchor pipeline under the maintenance flag must NOT silence credit-ledger integrity checks (it touches credit ledger tables, not `anchors`). **PII (§1.4):** logs + Sentry carry `org_id` + a COARSE divergence BUCKET only (`bucketDivergence()` → `'0' | '±1-9' | '±10-99' | '±100-999' | '±1000+'`) — never raw `balance`/`granted`/`ledger_sum`/`expected`/`divergence` (raw credit amounts are PII; **C2:** when `expected==0`, `divergence==balance`, so even the raw divergence value would leak the org's exact balance — hence the bucket); per-org alert rows capped at 50. Probe failure (RPC error/throw) → `healthy:false` with `divergedCount:null` so a broken read never masquerades as a clean report. Cron route `/jobs/reconcile-credit-conservation` (detected drift → 200, no Scheduler retry; probe failure → 500). In-process backup in `scheduled.ts` (daily `0 9 * * *`). Pure fn + cron-glue shape mirrors `stuck-anchor-monitor.ts`. **Mig 0347 is T3 + prod-apply-first (Carson-gated, like 0343); scheduler/deploy binding is also T3 + serialized under Carson** — this PR builds + tests only.
