...
| **FCRA counsel** (SCRUM-883, gates Q4.1 NVI) | Founder + CTO-approved legal/business operator | before any NVI/background-screening feature | **Needs Human** — Foley & Lardner **privacy** intro held 2026-06-02/03 (Peter Stockburger), **not FCRA-specific** | Confirm FCRA scope with counsel (or engage FCRA-specialist) under the approved legal/business route | Foley thread (`19e8572e…`), notes doc `11T8N1U…` |
...
- **CE key → Secret Manager (S0-7.2)**: `Credential_Engine` already has inventory labels and a CE-specific worker-SA accessor binding. Lane 1 owns any broader Secret-Manager/IAM estate decision; Lane 3 owns the CE relationship. No rename, two-secret split, migration, or retirement is directed by this tracker.

## Notes

- **Adjacent (not one of the five):** AUDD grant (Australian fintech) application in progress — track separately, not a launch gate.
- **CSA STAR L1** (separate from CASA/SOC2): 197-control CAIQ pack, prod 107/107 RLS verified, awaiting CTO-approved compliance review and approved-operator submission (v2 sheets regenerated 2026-06-17). This submission is not added to the founder-reserved list. CSA STAR **L2** is the Q2.6 roadmap epic (SCRUM-712).
