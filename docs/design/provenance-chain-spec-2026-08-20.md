...
1. **Close every projection before feeding the channel.** Ship a migration pinning the anonymous `recipient_identifier` to `''` permanently, moving HMAC computation to relationship-scoped surfaces only. §1.8 permits a frozen field to narrow; a permanent empty string is a no-statement and therefore allowed.
   *Justify this as "close before feeding," not by row counts.* Measured: 0 of the 50,000 most recently created non-deleted anchors carry `metadata->>'recipient'`, and 0 of 17 connector anchors do. The flip is near-empty **today** — which is exactly why a reviewer under time pressure might drop the gate as theatre. Phase 3 is what populates it.
