...
| `credential.verified` | An `/api/v1/verify/*` call resolves a credential as `SECURED` / `PENDING` / `REVOKED` / `EXPIRED` | Contract defined; emit-point implementation in active development |
...
**Contract-defined, emit-point pending:** the payload schemas, dispatch validation, and HMAC signing for `credential.*` events are live in this release. You can register webhook subscriptions for them today; deliveries begin once the per-event emit points land in follow-up Phase-2 stories. The schemas obey the same allowlist rules as anchor events: `public_id`-only, no internal UUIDs, no fingerprint, ISO 8601 timestamps. See `services/worker/src/webhooks/payload-schemas.ts` for the canonical contract.
...
|---|---|---|---|
| v1.1 | 2026-05-01 | SCRUM-1445 | Added database-level `public_id` fields for webhook endpoints (`WHK-{org_prefix}-{16}`) and delivery logs (`DLV-{16}`) as the prerequisite for a future v2 public-ID-only webhook API. |
