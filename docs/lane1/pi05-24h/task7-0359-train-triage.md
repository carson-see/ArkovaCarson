...
- **Concurrency-safe index builds:** every index candidate (0361/0363) should be authored with `CREATE INDEX CONCURRENTLY` — but that **cannot run inside the `BEGIN;…COMMIT;` migration wrapper**. Flag for the Jul-22 authoring: these need the split-transaction migration pattern (or a follow-up maintenance step), which affects how they soak. This is a real authoring constraint the CTO/RTE should note now.
