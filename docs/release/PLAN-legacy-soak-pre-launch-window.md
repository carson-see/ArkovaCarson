...
**Bottom line, stated up front: this audit found zero genuinely unknown or unauthorized actors.** Every non-Carson, non-bot identity in the pre-cutoff commit history reconciles to either a founder of Arkova (`BestNessie`, §3.4) or a former contractor no longer with the company (`prajalsharma`, §3.3); the one remaining flagged string (`Arkova SCRUM-894`) is a shared local git-config artifact, not a person (§3.5). That is a clean, good result, and this section states it plainly rather than leaving any implication of mystery or misconduct.
...
| **`prajalsharma <prajalsharma1120@gmail.com>`** | **7** | **Former contractor — see §3.3** |
| **`BestNessie <129661809+BestNessie@users.noreply.github.com>`** | **6** (direct-author commits; more when counted by PR authorship, see §3.4) | **Founder of Arkova — see §3.4** |
...
Nine of the fourteen distinct identity strings above are Carson under different local git configurations or GitHub noreply aliases (2,116 commits combined) — an identity-hygiene footnote worth a one-line fix (a single canonical `user.email` per machine) but not a provenance risk. `dependabot[bot]` and `mergify[bot]` are automated and out of scope for a human-provenance audit. That leaves two identities that needed resolution beyond a bare `git log` read and one resolved false positive: `prajalsharma` resolves to a former contractor (§3.3) and `BestNessie` resolves to a co-founder of Arkova (§3.4) — both authorized, neither unknown.

### 3.3 `prajalsharma <prajalsharma1120@gmail.com>` — former contractor

Seven commits, all dated **2026-01-29 to 2026-02-19** — the very first commits in the repository's history (`first commit`, `Initial Arkova MVP setup`, `Initial Arkova MVP codebase`, plus the rename to "Arkova" and a UI redesign pass). This is the original project scaffold. Carson has confirmed directly that prajalsharma was a contractor engaged for the original build under a signed agreement and is no longer with the company. This is a known, authorized identity — not an unknown or unauthorized actor.

**Current GitHub standing:** the login implied by the commit email (`prajalsharma1120`) does not resolve via `gh api users/prajalsharma1120` (404 — account not found or renamed) as of 2026-07-28, and **the identity is not a current repository collaborator** (absent from `gh api repos/carson-see/ArkovaCarson/collaborators`). Both facts are consistent with an ended contractor engagement, not with anything suspicious — access that stopped when the work stopped.
...
- The largest single surviving block, `package-lock.json` (4,511 lines), is a generated lockfile and carries no independent authorship risk or IP-exposure concern.

**Why this code is still in scope for replacement — two independent reasons, both real:**

1. **Staleness / no current owner.** The code is unowned and has not been re-verified by anyone since the contractor's engagement ended — nobody on the current team wrote it, reviewed its edge-case reasoning at the time, or has since revisited it with fresh eyes. The two RLS test files and the shared validator module are the genuine higher-stakes surviving artifacts and should be explicitly re-reviewed (not merely re-run) as part of this soak's provenance closure, since RLS-test-writer intent (what edge cases did the author think to cover) is exactly the kind of thing a passing test cannot self-certify.
2. **Legal / IP-exposure reduction (Carson, 2026-07-28): "we want to replace his code for legal reasons, he's signed contracts, but still."** The contractor engagement was under a signed agreement (presumably including IP assignment), so this is not a claim that the code is legally unowned — it is a normal, prudent business posture ahead of diligence, an audit, or a funding event: reduce reliance on a third party's contractual assignment holding up under scrutiny by minimizing how much shipped product still consists of a former contractor's original code, regardless of whether any specific line is risky on the merits. This is a business/legal risk decision made by the founder, not an accusation about the individual or a claim that the current agreement is deficient.

Both reasons point the same direction and reinforce each other, but they imply different scopes, and this document does not decide between them — that is an open call for Carson, laid out honestly below.

**Open decision for Carson — replacement scope, not yet decided here:**

| Option | What gets replaced | Line count | Cost / risk |
|---|---|---|---|
| **(A) Risk-weighted replacement** | The REPLACE-not-review subset only: `tests/rls/p7.test.ts` (537) + `tests/rls/rls.test.ts` (457) + `src/lib/validators.ts` (260) | **1,254 lines** | Cheap, fast, contained to the highest-blast-radius files. Fully addresses the staleness rationale. **Does not fully discharge the legal/IP rationale** — it leaves roughly 18,900 lines of prajalsharma's original code in the shipped product. |
| **(B) Full clean-room replacement** | Every surviving prajalsharma-attributed line, minus the generated `package-lock.json` (4,511 lines, not hand-authored, not a meaningful IP object) | **~15,681 lines** of hand-authored code (20,192 total − 4,511 lockfile), including Option A's 1,254 plus the auth-adjacent UI (SignUpForm 174, LoginForm 141), billing UI (BillingOverview 221), proof-package code (proofPackage.ts 156), the already-dead `CSVUploadWizard.tsx` (448 — deletion, not rewrite), and an additional **~13,287 lines** spread across the remaining files in the top-40-by-surviving-lines list plus the long tail beyond it (not itemized in this document; would need its own full-file enumeration pass to quantify precisely, file by file) | Materially more expensive and slower than (A). Fully discharges the legal/IP concern by minimizing shipped surface, but carries its own risk this document's own §3.7/§5-item-5 already name: rewriting stable, working code under time pressure introduces new, unsoaked bugs into code that was at least functioning. |

**This document does not pick between (A) and (B).** A licensed attorney should confirm what level of replacement, if any, is actually required — whether the signed contractor agreement's IP assignment already discharges the concern on its own is a legal question, not an engineering one, the same posture this release plan already takes on the LGPL item elsewhere in `RELEASE-PLAN-2026-08-FINAL.md`. Engineering can execute either option once the legal question is answered; it should not choose the scope on legal grounds it isn't positioned to evaluate. Until that answer lands, treat (A) as the soak's working floor (§3.7 already schedules it as REPLACE-not-review regardless) and (B) as pending counsel's input, not yet authorized or scheduled.

### 3.4 `BestNessie <129661809+BestNessie@users.noreply.github.com>` — founder of Arkova

Six direct-author commits (2026-04-25 to 2026-04-27); PR authorship (`gh pr view <N> --json author`) shows this identity as the actual GitHub author on six merged PRs in the same window: #568, #570, #571, #572, #573, and #1041 (the last one recovered/re-landed later by Carson, per commit `recover(CSI-04C/04D)`). `gh api repos/carson-see/ArkovaCarson/collaborators` shows `BestNessie` with `push: true` access as of 2026-07-28.

**Resolved directly by Carson, 2026-07-28, in chat, during this correction: `BestNessie` is a founder of Arkova.** This is not an unknown actor, a provenance risk, or a replace-not-review target because of who wrote the code. Her push access is rightful — it is a principal's own access to the company's repository — and her commits, PRs, and the three migrations below are authorized work by a principal of the company, not by an outside or unverified party. The account's sparse public GitHub metadata (no bio, no visible org affiliation via the public API) reflects nothing more than personal profile settings and should never have been read as a provenance signal in the first place.

**Files touched, all still tracked on `main`:** `.github/workflows/ci.yml`, `services/worker/src/api/v1/{ai-extract,anchor-evidence,anchor-lifecycle,batch,verify,webhooks}.ts` and their test files, `services/worker/src/jobs/batch-anchor.ts`, `services/worker/src/webhooks/{delivery,replay}.ts`, `src/tests/security/{audit-06-payment-ledger-invoker,audit-07-empty-policy-tables,audit-08-search-path-coverage,service-role-audit}.test.ts`, and three migrations: `0273_audit08_function_search_path_public.sql`, `0274_audit06_payment_ledger_security_invoker.sql`, `0275_audit07_empty_policy_tables.sql`.

**Why these migrations are still in this soak's scope — and why that has nothing to do with authorship:** the three migrations pin `search_path=public` on mutable functions, flip `payment_ledger` to SECURITY INVOKER, and add deny-all RLS policies to seven tables — exactly the security-boundary category §3.1's blast-radius rule and §3.7's REPLACE-not-review discipline exist for. They are in scope for the same reason every other pre-2026-06-13 SECURITY DEFINER/RLS change is in scope (§1.4, §4.3): they are old, they predate the review discipline built since (§2a), and this soak's standing practice is a from-scratch adversarial re-review of security-boundary code regardless of who wrote it — not a response to authorship. `ai-extract.ts` and `batch-anchor.ts` get the same treatment for the same reason (they sit on the same risk surface named in §4.3), not because this identity touched them.

**Correction note:** an earlier version of this section flagged `BestNessie` as an unresolved provenance risk and recommended a named, dated identity/access decision from Carson. That recommendation is withdrawn in full — it was based on an authorship signal (`git log`/`gh api` collaborator visibility) being mistaken for an authorization signal, which is exactly the methodological gap named in §3.8 below. There is no outstanding identity or access question here.

### 3.5 `Arkova SCRUM-894 <noreply@arkova.ai>` — false positive, resolved

Traced via `gh api search/issues` against each commit SHA and `gh pr view` on the resulting PR numbers: commits under this local identity correspond to real PRs authored by both `carson-see` (e.g. #1242) and `BestNessie` (e.g. #1041), meaning **this is a shared local `git config user.name` string used across sessions/machines, not a distinct person.** No separate provenance action needed beyond the general identity-hygiene note in §3.2 — folded in here rather than treated as a third unknown actor, since treating it as one would double-count `BestNessie`'s already-attributed founder work under a different label.
...
For the items in scope in §3.3 and §3.4 — the RLS test files and `validators.ts` (legacy, unowned since the contractor's engagement ended, and, pending §3.3's open legal-scope decision, potentially in scope for IP reasons too), and the three search-path/RLS migrations and their adjacent worker call sites (legacy security-boundary code in scope under §3.1's blast-radius rule regardless of authorship) — the recommended action is not a deletion-and-rewrite-from-scratch project before the soak can start (that would itself be new, unsoaked code introduced under time pressure — see pre-mortem item #6 below). It is: (1) a from-scratch adversarial re-review by someone who did not author the original code, treating it exactly as if it arrived today via an untrusted PR; (2) any defect found gets fixed as its own tracked change, soaked and reviewed like any other fix; (3) only if a re-review finds the code fundamentally unsound (not just improvable) does a full replacement get scheduled, and that replacement is itself new code requiring its own soak, not a shortcut around this soak. This keeps the REPLACE label meaningful (an escalation path exists) without turning "the code is old" or "the original author has moved on" into an automatic rewrite mandate that would itself introduce risk. Where §3.3's Option (B) full clean-room replacement is ultimately authorized (pending counsel's input), the same three-step discipline applies to that larger scope — it does not get a shortcut just because the driver is legal rather than technical.

### 3.8 Methodological lesson: authorship is not authorization

This audit's first draft misclassified a founder as an unresolved or flagged actor purely on the strength of `git log`/`git blame`/`gh api` collaborator-metadata signals. Those tools establish **authorship** — who committed what, when, with what access level at the time — with high confidence. They establish nothing about **authorization** — whether the person behind that identity is a principal, an employee, a contractor, or a stranger. An account with sparse public GitHub metadata (no bio, no visible org) looks identical whether it belongs to a co-founder using a personal handle or an actual unknown actor; the signal cannot distinguish the two cases, and treating "the public API can't tell me who this is" as "this is unresolved" was the error this document made in its first version, now corrected.

**Standing rule for any future provenance pass:** reconcile every non-bot author identity against a known-principals/known-contractors list (starting point: `memory/project_art_roster.md`, plus direct confirmation from Carson for any name not already on it) **before** assigning any risk classification, flag, or recommended action to that identity. If the reconciliation step is skipped, "unknown" defaults to a factual claim about the world (nobody knows who this is) that the audit has not actually established — it has only established that a public API lookup returned no name. Misclassifying a colleague or a former contractor as an unknown/unauthorized actor is not a neutral placeholder error; it produces a document that reads as an accusation and can prompt access or trust decisions against the wrong person for the wrong reason. Get confirmation first, every time.
...
**Same 10,000-daily-user load standard as the launch soak, not a lighter profile.** Carson, 2026-07-28: "10k daily users for the legacy code during soak as well." This soak reuses the launch soak's derived load model verbatim (`72h-soak-runbook-2026-08.md` §0.6) rather than re-deriving a separate one: ~25,000 anchors/day, ~2.8 anchor-creates/sec sustained and ~8.3/sec burst, ~28 RPS sustained / ~83 RPS burst, ~250 peak concurrent sessions, with the same p95/p99 budgets per critical path. Same rig sizing too — medium tier, not micro/small; an undersized rig produces latency numbers that are artifacts of the rig rather than the code, which makes the evidence worthless.

**Why this matters more for legacy code, not less:** the pre-2026-06-13 subsystems (worker's 365K legacy lines, edge at 92.1% legacy, integrations at 98.6% legacy) have never been load-tested at target scale — §2(b) already established that CI's silent job-tail skip means their historical "green" checks overstated coverage for this entire era, and that same era has also simply never run at anything close to 10k-DAU volume. Old code that has only ever run at today's low real-world volume is exactly where a scale ceiling hides silently until it's hit in production. The launch soak already surfaced one architectural ceiling on paper this way — the nightly flush capping at `BATCH_ANCHOR_MAX_SIZE=10000` against ~25k anchors/day (`72h-soak-runbook-2026-08.md` §0.6) — and the legacy soak should be expected to surface more of that class precisely because nobody has ever pushed this code this hard before. A lighter load profile "because it's old code" would be backwards: it is exactly the code least likely to have been proven at scale, so it is the code the full load standard matters most for, not least.

**Resourcing consequence, stated plainly:** running at full 10k-DAU load means this soak cannot share a rig with the launch soak and cannot be run on a cheaper tier — it needs its own medium-tier rig, sized the same as the launch soak's, provisioned separately. Cost is one extra medium-tier rig for roughly the duration of both soaks — negligible against the 2026-08-10 schedule. Separate rigs mean the two soaks do not contend for infrastructure; §6 covers the resulting parallel-start timing, which depends on the launch soak's early gate rather than on rig cost.
...
**Mechanism:** §3's surviving finding — prajalsharma's surviving RLS test scaffolding and validator module, correctly attributed to a former contractor rather than an unknown actor (§3.3), plus the still-open replacement-scope decision (§3.3's Option A/B table) — is exactly the kind of finding that's easy to file and then never revisit, especially once the soak itself starts consuming all the attention. `POSTMORTEM-sprint-2026-07-28.md` §5 already names "findings-per-hour" and "time-to-detect" as metrics this org doesn't currently track — the same gap applies to "findings acted on vs. filed."
**Earliest signal:** the T+72h (or final) gate closes with §3.3's REPLACE-not-review items (the two RLS test files, `validators.ts`) still unreviewed, or with the Option A/B legal-scope question still unanswered, and no explicit decision recorded either way.
**Mitigation:** track §3.3's REPLACE-not-review items and its open Option A/B scope question as their own line items, each with an owner and a deadline independent of the soak's own clock, per §3.7's three-step discipline. The legal-scope question specifically needs counsel's input before it can be closed (§3.3) — that is a named dependency, not a silent stall. **Owner:** RTE (the code-level REPLACE items in §3.7); Carson, routed to counsel, for the Option A/B legal-scope decision (§3.3).

### 5. Replacement work introduces new bugs in code that was at least stable

**Mechanism:** §3.7 already names this risk directly — a rewrite mandate driven by "the original author has moved on" or "for legal reasons" rather than by an actual defect found on re-review would introduce new, unsoaked code under the same time pressure this whole two-soak structure exists to relieve. New code is definitionally not covered by either soak's frozen-head model until it goes through the launch soak's own process. This risk applies with extra force to §3.3's Option (B) full clean-room replacement, which is by construction the larger and more bug-prone of the two options if it is ever authorized.
**Earliest signal:** a REPLACE-not-review item escalates straight to "rewrite this" without a documented adversarial re-review finding a concrete defect first, or Option (B) gets scheduled without the counsel sign-off §3.3 says it needs.
**Mitigation:** §3.7's three-step discipline (re-review as if untrusted → fix what's found, soaked like any other change → full replacement only if re-review finds it fundamentally unsound) is the standing mitigation; the risk here is skipping step 1 under pressure, or skipping counsel's input on scope under pressure. **Owner:** RTE enforces the sequence; CTO-delegate signs off before any replacement (not just a fix) is scheduled; Carson/counsel sign off before Option (B) specifically is scheduled.
...
**Earliest signal:** any `gcloud`/Supabase command touching the launch soak's rig (`launch-72h-2026-08`) issued in service of this soak's provisioning or exercises; or both soaks' monitors alarming at the same moment with only one set of eyes available.
**Mitigation (CTO ruling, 2026-07-28, superseding an earlier T+24h-wait draft of this section — Carson: "why not spin up another rig, we are on a clock, what would +24 hours mean"):** infrastructure contention is fully resolved by provisioning a genuinely separate rig — that alone satisfies §0.5's rule, and rig cost (one extra medium-tier rig for roughly the duration of both soaks) is negligible against the 2026-08-10 schedule. The remaining, real dependency is not rig cost, it's the launch soak's own T+0–2h smoke gate: if the launch soak fails that gate, it aborts and restarts, and a second soak burning time against a head that's about to be invalidated is wasted effort, plus that early window is exactly when catching an abort signal matters most and attention must not be split. Once the launch soak is past T+2h and genuinely exercising the system, the legacy soak starts on its own separate rig — see §6 for the full sequencing and the attention-priority rule for when both soaks need eyes at once. **Owner:** RTE.
...
**Mechanism:** §3's own framing note exists because this is a real risk — an earlier draft of this document read "BestNessie authored the RLS-hardening migrations and still has push access" in a way that, out of context, could be mistaken for an accusation. It was factually wrong besides (BestNessie is a founder, §3.4), which makes the underlying risk even sharper: a provenance document that skips authorization reconciliation before writing a risk sentence can misfire twice over — once by tone, once by fact.
**Earliest signal:** this document, or any summary of it, gets cited without §3's opening framing paragraph and zero-unknown-actors conclusion, or gets shortened into a headline like "unknown contributor security risk."
**Mitigation:** keep §3's framing attached whenever this document's provenance findings are cited elsewhere (Jira, Confluence, the SOC2 evidence pack). The finding is "confirm and decide" (or, now, "confirmed — zero unknowns"), not "something was found wrong." **Owner:** whoever cites this document downstream — stated here so the obligation travels with the content.
...
**CTO ruling, 2026-07-28, superseding an earlier draft of this section that gated the legacy rig on the launch soak reaching its own T+24h gate.** Carson pushed back directly: "why not spin up another rig, we are on a clock, what would +24 hours mean" — and the earlier figure was over-conservative. The corrected constraint below reflects that.
...
### 6.2 The two soaks run in parallel, on separate rigs, gated on the launch soak's smoke gate rather than its full duration

**The legacy soak's rig is provisioned as its own, genuinely separate rig, and the legacy soak starts as soon as the launch soak clears its own T+0–2h smoke gate — not after T+24h, and not after the launch soak completes.**

Why this is safe and why it isn't a shortcut:

- **Separate rigs eliminate the infrastructure-contention half of the constraint outright.** The original non-contention rule (`72h-soak-runbook-2026-08.md` §0.5, pre-mortem item #7) is about not sharing a rig; a second, independently provisioned medium-tier rig satisfies that fully. Cost is one extra medium-tier rig for roughly the duration of both soaks — negligible against the schedule below.
- **The real dependency was never rig cost — it's the launch soak's smoke gate.** If the launch soak fails its T+0–2h gate, it aborts, gets fixed, and restarts on a new head. Starting the legacy soak before that gate clears risks burning soak-hours against a launch-soak state that's about to be invalidated, and the T+0–2h window is also the single highest-attention period for catching an abort signal — splitting attention across two soaks' opening gates at once is the one place serialization still matters. Once the launch soak is past T+2h and demonstrably exercising the system for real, that risk is gone and the legacy soak can start on its own rig without waiting further.
- **The schedule cannot absorb full serialization.** The chain is: 72h launch soak → independent pen test → fix-all → 7-day full-application soak → release, targeting 2026-08-10. That is already tight. Serializing the two 72h soaks (waiting for the launch soak to fully complete, or even to clear a T+24h gate, before starting the legacy soak) removes roughly a day from the fix-all window — precisely the buffer this plan most needs to protect, since pen-test findings are unpredictable in size and the fix-all step is the one stage in the chain without its own fixed duration.

**What still holds unchanged from earlier drafts of this document:** both soaks run at the full 10k-DAU load profile (§4.4) on medium-tier rigs — no cheaper tier and no lighter load for the legacy soak because it's older code; each soak gets its own `clean_mirror` preflight before its own clock starts; each soak keeps its own provenance record (this document's §3 for the legacy soak, the launch runbook's own equivalent for soak #1); and the legacy soak keeps its own gate structure and abort criteria (§4.2) rather than silently inheriting or merging with the launch soak's — running concurrently does not mean running as one soak.

**Operational note — two soaks running concurrently means two sets of continuous monitors and two gate schedules, and that needs an explicit owner-and-priority split, not an assumption that one person watches both equally well:**

- The launch soak keeps its own dedicated monitor and gate schedule per `72h-soak-runbook-2026-08.md`'s own continuity-control discipline (§0.8), unchanged by this document.
- The legacy soak gets its own dedicated monitor and its own gate schedule (§4.2), starting from its own T+0 once it begins (which, per above, is timestamped to "launch soak cleared T+2h," not to a shared T+0).
- **If both soaks need attention at the same moment, the launch soak's abort triggers take precedence.** The launch soak gates the 2026-08-10 release; the legacy soak does not (§6.3). A legacy-soak signal that can wait a few minutes should wait; a launch-soak abort signal should not. This is a standing priority rule for the RTE (or whoever is on watch), not a judgment call to be made fresh under pressure each time it comes up.

### 6.3 Explicit non-goal

This document does not set a fixed legacy-soak start timestamp, duration, or rig name — the start is defined relative to the launch soak's T+0–2h gate clearing (§6.2), not as a fixed calendar time, because the launch soak's own clock has not started as of this writing (per `72h-soak-runbook-2026-08.md`'s own state as of this writing: no rig provisioned, no PR set frozen, soak clock not started). Setting a fixed date now would be exactly the kind of unverified forward claim `memory/feedback_never_claim_unstarted_work_as_in_progress.md` warns against. What this document commits to is the **plan, the scope, the provenance findings, the pre-mortem, and the trigger condition for start** — the same deliverables the launch soak had before its own clock started, plus the explicit parallel-start rule this CTO ruling adds. The legacy soak does not gate the 2026-08-10 release; the launch soak does.

---

_Written 2026-07-28, RTE/CTO-delegate session (SCRUM-2980 lane), per founder directive (initial: soak everything before 2026-05-01; revised same day to 2026-06-01; revised again same day to zero-gap against the launch soak's exact 45-day window — "fucking close that 12 day gap then"). Every quantitative claim above (surviving-line counts, subsystem percentages, author commit counts, the three verified corroborating-evidence claims, the coverage-guarantee count partition) was derived this session from live `git log`/`git blame`/`gh api` output against commit `595b1b730011e37801f55ef62357daf0b1d84bbb`, not carried forward from a prior session or from the founder's framing without independent verification. §3's author-provenance classifications were corrected later the same day, 2026-07-28, after Carson identified in chat that `BestNessie` is a founder of Arkova (not an unknown actor) and `prajalsharma` is a former contractor (not an unknown actor) — see §3.4's correction note and §3.8 for the methodological lesson this drove. §4.4's load model and §6/§7's sequencing were also corrected the same day: the legacy soak now runs at the full 10k-DAU standard (Carson) and starts in parallel with the launch soak, gated on that soak's T+0–2h smoke gate rather than its full duration or a T+24h checkpoint (CTO ruling). No rig has been provisioned for this soak. No soak clock has started. This document is the plan and pre-mortem only, per `72h-soak-runbook-2026-08.md` §0.5's own instruction for how soak #2 should be scoped before it runs._
