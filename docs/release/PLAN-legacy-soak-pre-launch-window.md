...
**Resourcing consequence, stated plainly:** running at full 10k-DAU load means this soak cannot share a rig with the launch soak and cannot be run on a cheaper tier — it needs its own medium-tier rig, sized the same as the launch soak's. This reinforces §6's existing sequencing constraint (the legacy rig waits until the launch soak clears its own T+24h gate) rather than changing it; it does not create a new constraint, it explains why the existing one is necessary on cost/capacity grounds as well as attention grounds.
