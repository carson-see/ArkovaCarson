...
Systematic triage of every open bug across the bug log + UAT reports + Jira cross-refs, then fixed the top-10 most pressing. All fixes shipped in one PR with unit tests, `/simplify` pass, and `/code-review:code-review` pass.

| ID | Severity | Summary | Fix | Regression Test |
|----|----------|---------|-----|-----------------|
| BUG-2026-04-21-001 | **HIGH** | `/compliance/scorecard` rendered "Audit failed with HTTP 403" for every individual-tier user because the worker's `/api/v1/compliance/audit` endpoint is org-scoped and the UI fired the request regardless of `profile.org_id`. Flagged in UAT 2026-04-18 Bug 1 (`docs/bugs/uat_2026_04_18_product_guide.md:25`). | `ComplianceScorecardPage` now detects `!profile.org_id` and renders an `OrgRequiredCard` empty-state with a "Create organisation" CTA instead of firing the fetch. | `src/pages/ComplianceScorecardPage.test.tsx` — new test asserts the empty state renders and the fetchFn is NOT called. |
| BUG-2026-04-21-002 | MEDIUM | `/settings/api-keys` leaked the engineering copy "Unable to load API keys. Ensure the worker service is running.: User must belong to an organization" to individual-tier users. UAT 2026-04-18 Bug 3 + UAT 2026-04-05 BUG-UAT5-04. | (a) Rewrote `API_KEY_LABELS.FETCH_ERROR` + `SYSTEM_HEALTH_LABELS.WORKER_HINT` in `src/lib/copy.ts` to drop the "worker service" language. (b) Added new `OrgRequiredCard` empty state for individual users — skips the `useApiKeys`/`useApiUsage` hooks entirely so no worker round-trips fire. | `src/pages/ApiKeySettingsPage.test.tsx` — new test asserts the friendly CTA, no "worker service" text anywhere on the page, and that the create-key / usage UI is hidden for individuals. |
| BUG-2026-04-21-003 | MEDIUM | `/settings` flooded the browser console + Sentry with HTTP 404s on `GET data_subject_requests?...` for every user in environments where the REG-11/-19 migration wasn't deployed yet (prod is on migration 0185; the table lands in the 0212+ tranche). UAT 2026-04-18 Bug 2. | `DataCorrectionForm.fetchRequests` now gates the Supabase query on `VITE_ENABLE_DSAR_UI` (strict truthy: `'1' \| 'true' \| true` — `'false'`/`'0'` / empty string all disable). | `src/components/auth/DataCorrectionForm.test.tsx` — two new tests assert no Supabase `.from('data_subject_requests')` call fires when the flag is empty or explicitly `'false'`. |
| BUG-2026-04-21-004 | MEDIUM | Public verify page rendered `ISSUED AT 2026-04-01T00:00:00Z` (raw ISO-8601) in the non-template metadata branch of `CredentialRenderer` for any metadata key that wasn't typed as `date` in the template schema. UAT 2026-04-05 BUG-UAT5-09 (observed on `/verify/ARK-ACD-Z9NMCY`, a REVOKED credential). | `formatFieldValue` now detects `/^\d{4}-\d{2}-\d{2}T/` and routes the value through the existing `formatDate` helper so any ISO datetime renders as "April 1, 2026" style en-US long form. | `src/components/credentials/CredentialRenderer.test.tsx` — new test asserts the raw ISO does not appear and the formatted date does. |
| BUG-2026-04-21-005 | LOW | Marketing docs at `arkova-marketing/src/pages/docs/ApiReferencePage.tsx:138` and `WikiPage.tsx:580`/`:584`/`:619` showed `app.arkova.io` / `api.arkova.io` in code examples (5 occurrences), contradicting the `.io → .ai` migration from Session 35 (BUG-S35-11 / SCRUM-534). | Replaced all 5 occurrences with `arkova.ai` equivalents. | Covered indirectly by `services/worker/src/lib/urls.test.ts` which asserts `.ai` on every `buildVerifyUrl` path. |
| BUG-2026-04-21-006 | LOW | `infra/cloudflare/load-balancer.ts` defaulted `WORKER_TUNNEL_HOSTNAME` → `worker.arkova.io`, `LB_HOSTNAME` → `api.arkova.io`, and `NOTIFICATION_EMAIL` → `ops@arkova.io` when the env vars were unset. Would silently misconfigure a fresh infra apply if someone didn't know about the rename. | Switched all three defaults to `arkova.ai`. | N/A (infra script, no unit test); follow-up operator action to confirm env vars are set in GCP before next apply. |
| BUG-2026-04-21-007 | LOW | `services/worker/src/lib/urls 3.ts` + `urls.test 3.ts` — macOS Finder / iCloud Drive " 3" duplicates of legitimate files were checked into the repo. CI test runner was double-executing the URL test suite; future edits to `urls.ts` would forget to mirror into the duplicate and silently diverge. | `git rm` both duplicate files. | URL test suite still passes 14/14 via the single `urls.test.ts`. |
...
| ID | Severity | Summary | Fix | Regression Test |
|----|----------|---------|-----|-----------------|
| BUG-2026-04-19-001 | **HIGH (prod regression)** | `/dashboard` `UsageWidget` stuck in perpetual loading skeleton in production for users who own many anchors (platform admin + high-volume pipeline operators). Root cause: `fetchEntitlementData` in `src/hooks/useEntitlements.ts` issued `supabase.from('anchors').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', monthStart)` through RLS. For carson@arkova.ai (757K+ anchors this month via the pipeline), `EXPLAIN ANALYZE` confirmed 23.4s / Index-Only Scan touching 323K buffers; Supabase REST 500-abort at 30s. React Query default retries stacked up to 90+s of stuck skeleton. Found by 2026-04-19 prod UAT: Chrome MCP screenshot of `arkova-26.vercel.app/dashboard` showed a third-slot card with 3 `animate-pulse` skeletons between `Compliance Score` and `Credits` that never resolved, then JS hook of `window.fetch` captured `[BLOCKED redacted supabase URL] status=500 ms=30073`. | **First fix (PR #426, 2026-04-19 AM):** AbortController 5s timeout + try/catch fallback to `{ count: 0 }` + `console.warn`. Verified live in prod — skeleton gone, widget renders "0 records this month / Unlimited." **Proper fix (PR #427, 2026-04-19 PM — this branch):** migration 0220 adds `get_user_monthly_anchor_count` — SECURITY DEFINER RPC mirroring the existing `get_user_anchor_stats` pattern used by the dashboard stat cards. Bypasses RLS (which was the slow path), enforces per-user isolation internally (returns 0 if `p_user_id != auth.uid()`), applied to prod via Supabase MCP. `useEntitlements.ts` rewired to `supabase.rpc('get_user_monthly_anchor_count', { p_user_id: userId })` instead of the 5s abort hack. | `src/hooks/useEntitlements.test.ts` — regression test `falls back to recordsUsed=0 when anchor count RPC errors (BUG-2026-04-19-001)` asserts `recordsUsed=0 + planName preserved + error=null + canCreateAnchor=true + console.warn emitted with exact error message`. Full suite: 15/15 passing. Migration 0220 smoke-tested in prod (`get_user_monthly_anchor_count('<other-uid>')` returns 0, RLS isolation verified). |

## 2026-04-18 — Top-10 Sprint Bug Finds

While working SCRUM-893/899/900/901/902, **five** defects were found + fixed — two of them were HIGH-severity prod regressions caught by a real UAT session against `arkova-26.vercel.app`. Same entries logged in the master [Bug Tracker Spreadsheet](https://docs.google.com/spreadsheets/d/1mOReOXL7cmBNDD77TKVKF3LsdQ3mEcmDbgs5q_pTEk4/edit?gid=0#gid=0).

| ID | Severity | Summary | Fix | Regression Test |
|----|----------|---------|-----|-----------------|
| BUG-2026-04-18-001 | HIGH | Sales doc + 7 compliance docs claimed "AWS KMS" (or "AWS + GCP KMS") for Bitcoin treasury signing — production has no AWS account, only GCP. Risk: customer procurement would see we can't deliver what we promised. Source: SCRUM-902 audit. | Replaced references across `docs/wiki/TECHNICAL_SECURITY_WIKI.md`, `docs/wiki/generate-wiki-docx.cjs`, `docs/compliance/{kenya,malaysia,uk-cyber-essentials}/*`, `docs/compliance/tabletop-exercise-2026-04-12.md`, `docs/security/launch_readiness_security_audit.md`, `docs/confluence/14_kms_operations.md`. Source-file headers flag AWS provider as code-level only (non-production). | N/A (doc-only). `memory/feedback_no_aws.md` codifies the rule going forward. |
| BUG-2026-04-18-002 | MEDIUM | Frontend test `complianceMapping.test.ts > "all controls have valid framework values"` out-of-date — missing LGPD, PDPA, LFPDPPP, EU-US DPF after INTL-01..03 + TRUST-03 added those frameworks. Test was failing on main before this sprint. | Updated `validFrameworks` array in the test with a comment noting new frameworks must also land there. | `src/lib/complianceMapping.test.ts:127` now green; `complianceMapping.test.ts` passes 19/19 tests. |
| BUG-2026-04-18-003 | LOW | Unresolved merge conflicts existed on branch `claude/charming-cori-NJeI4` at session start — conflict markers across `CLAUDE.md`, `services/worker/src/api/v1/compliance-audit.ts`, `services/worker/src/api/v1/compliance-audit.test.ts`. Would have blocked tests + merge if not resolved. | Merge resolution committed in `f5d3f05b` (accepted HEAD's newer work on compliance-audit; CLAUDE.md stats synced). | Post-resolution: `npx tsc --noEmit` clean both frontend + worker; full worker suite 3,409/3,409 tests passing. |
| BUG-2026-04-18-004 | **HIGH (prod regression)** | `/api/v1/compliance/audit` returned 503 `ADES_SIGNATURES_DISABLED` in production for every authenticated caller. Root cause: `router.use('/', adesSignatureGate(), …)` at `services/worker/src/api/v1/router.ts:333` and `:336` mounted the AdES feature gate at the root of the v1 router (to support the sub-routers' internal `/signatures/*` paths), but the gate didn't path-check its own request, so it 503-ed every `/api/v1/*` hit whenever `ENABLE_ADES_SIGNATURES` was off. Found during 2026-04-18 prod UAT of SCRUM-893 NCA-FU1 item #6 via Claude-in-Chrome MCP against `arkova-26.vercel.app/compliance/scorecard` — page rendered the error state "Audit failed with HTTP 503" and `fetch('/api/v1/compliance/audit?limit=10')` returned the `ADES_SIGNATURES_DISABLED` body. | Added a path-prefix allowlist inside `services/worker/src/middleware/adesFeatureGate.ts` (`ADES_PATH_PREFIXES = ['/sign', '/signatures', '/verify-signature']`) + `isAdesPath()` guard that falls through to `next()` for every non-AdES path. Preserves the intended gate for `/sign`, `/signatures/*`, and `/verify-signature` while removing the collateral 503. | New `services/worker/src/middleware/adesFeatureGate.test.ts` — 5 tests covering path-guard truthiness, prefix-collision rejection (`/signaturesX`), flag-off-AdES → 503, flag-off-non-AdES → passthrough, flag-on-AdES → passthrough. **RESOLVED** via PR #419 (squash `4559aa47`, Cloud Run deployed 2026-04-18 15:49 UTC). |
| BUG-2026-04-18-005 | **HIGH (prod regression)** | After BUG-2026-04-18-004 was fixed the scorecard + audit button still 401'd because `src/pages/ComplianceScorecardPage.tsx` and `src/components/compliance/AuditMyOrganizationButton.tsx` used raw `window.fetch(…, { credentials: 'include' })` without attaching the Supabase JWT. The worker's `requireAuth` middleware rejects any `/api/v1/compliance/audit` call that doesn't carry a `Authorization: Bearer <supabase-access-token>` header. Result: the scorecard has been broken in production for every user since it shipped in PR #411/#414 — pages always rendered "Audit failed with HTTP 401". Found by the same 2026-04-18 prod UAT session, immediately after the AdES fix landed. | Added a `fetchWithSupabaseJwt` helper to both components that reads `supabase.auth.getSession()` and attaches the Bearer token. Default `fetchFn` prop now routes through the helper in the browser; tests remain unaffected (they inject their own `fetchFn`). | Frontend tests for both components were already green with injected fetch; no test-contract change needed. Post-deploy UAT will verify the scorecard returns 200 + renders the gauge. |
...
| BUG-PROD-002 | CRITICAL | Google OAuth: "Unsupported provider: provider is not enabled" | Supabase config + code | **PARTIAL** — AuthCallbackPage.tsx created (was raw Navigate causing race condition). Still needs: enable Google provider in Supabase Dashboard + add OAuth Client ID/Secret |
