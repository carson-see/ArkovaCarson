...
| [FD-PROD-1](FD-PROD-1-0386-merged-but-unapplied-fingerprint-oracle-open.md) | **RESOLVED 2026-08-22.** 0386 is applied in prod (ledger head 0409) and the live function carries `AND a.status = 'SECURED'` — the fingerprint oracle is closed. Kept for its method: **test the predicate, not the hash** — `pg_get_functiondef` normalises, so md5/length are not stable identifiers. |
