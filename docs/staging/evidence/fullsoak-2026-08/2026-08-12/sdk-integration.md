...
> Run `2026-08-12T17:09:22Z` · rig `https://arkova-worker-fullsoak-2026-08-staging-270018525501.us-central1.run.app` · Supabase `gnkuaywlpmsaezwvlvhk`
...
> Repo HEAD `36faca5b88f4e8b29c43bb4d4267689e0bd52330`
...
| `C1h` | `Arkova.query()` (Nessie) fails CLOSED, not 200-with-empty | error / explicitly disabled | HTTP 200 + {"results":[],"count":0,"query":"what is my compliance posture"} | **FAIL** |
...
| `C2e` | `nessie_ask` MCP tool fails CLOSED, not a synthesized empty answer | error / explicitly disabled | {"content":[{"type":"text","text":"{\n  \"answer\": \"No relevant verified documents were found for your query.\",\n  \"citations\": [],\n  \"confidence\": 0,\n… | **FAIL** |
...
| `D2b` | The published model parses the live `/api/v1/verify/{id}` payload | parses | REJECTED on [compliance_controls]: compliance_controls | **FAIL** |
| `D3` | `Arkova.get_record()` reads the v2 record projection | a record | public_id='ARK-2026-9476A947' verified=True status='ACTIVE' fingerprint='d403127d80c480043717d7d047833799613bfe64c97f55548654eadba15eb8af' title='api-d403127d80 | **PASS** |
| `D4` | `Arkova.search()` returns a search envelope | an object | results=[] next_cursor=None | **PASS** |
| `D5` | `Arkova.list_orgs()` reads the public org index | an org list | organizations=[Org(public_id='gfawe273bczm', display_name='Acme Corp', domain='acme.example.com', website_url='https://acme.example.com', verification_status='V | **PASS** |
...
**TypeScript legs are worktree builds** (worktree 36faca5b8 — the npm packages are unpublished (Phase A)). That is the §5.1 S12/S13 false-pass
...
`SDK_INTEGRATION: 22 pass / 7 fail / 0 skip — FAIL`
