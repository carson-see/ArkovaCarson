...
- **`429-limiter-map-s33.md`** (L2-S0/L2-A, Sprint 3.3) — the five-bucket 429 attribution map: every 429 emitter in the worker with in-tree-verified `file:line`, the two dead-code claims (perOrgRateLimit unmounted; x402 payer limiter orphan), the L2-A fix that safely preserves and classifies upstream status/`Retry-After`, pathname-only client evidence, and request-level coalescing only from explicit bounded retry-loop attempt identity (never timestamp inference). Duplicate/gapped/out-of-range attempts and reused correlation IDs fail closed. The map also defines the strict header+log/upstream-event attribution contract and exit-criterion-3a bucket list (CTO memo R2 + Wave 2 alignment). Its `Claims ledger` table is machine-read by `scripts/ci/check-429-limiter-map.test.ts`, which fails CI when the tree drifts from the map.
