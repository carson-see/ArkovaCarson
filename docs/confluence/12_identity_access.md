...
`GET /api/v1/attestations/{publicId}` is a public verification endpoint for attestation records. The default response is intended to preserve the v1 compatibility shape while PR review and smoke verification close out. Evidence metadata is returned as a public array, and attestor credential lineage is only added when callers request it with `?include=credentials`.

### Public Evidence Metadata

Evidence rows expose public metadata only:

- `public_id` uses the `AEV-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX` format.
...
Each chain item includes `public_id`, `credential_type` (nullable), `status`, `fingerprint` (nullable), `version_number` (nullable), `parent_public_id` (nullable), `is_current`, `chain_proof` (nullable), and `record_uri`.
