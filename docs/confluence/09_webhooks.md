...
| 2026-04-12 | NCE-08/09 | Added `compliance.document_expiring` (cron-dispatched, 90/60/30/7-day windows via `check-credential-expiry` cron job, batched with Promise.allSettled). Added `compliance.gap_detected` (fires when new gaps found after score recalc). Gated behind `ENABLE_EXPIRY_ALERTS` flag. |
