...
| `evidence/s33-heldout-corpus-zero-overlap-index.json` | Public 621-row identity/content index bound to held-out head `c9a23019…` and admitted v7.1 head `251d32b9…`; proves 621/621 unique held-out ids/content and zero exact-id/normalized-content overlap with the 865/96 v7.1 exports. |
| `evidence/s33-heldout-corpus-authentication-v1.request.json` + `.signing-bytes` + `.signature.json` | Canonical public corpus-only detached-signing request, exact raw bytes, and separately verified Ed25519 signature under `arkova.s33.release-corpus.ed25519.v1`; final RC remains explicitly pending and the envelope grants no rig/soak/spend authority. |
