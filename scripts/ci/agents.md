...
## Files
- **`check-staging-evidence.ts`** — enforces CLAUDE.md §1.11 / §1.12 staging soak evidence on every PR. Path-based detector classifies the touched files into Tier T1/T2/T3, then verifies the PR body declares the required tier and includes the matching required fields.
  - **`isStagingToolingOnly()` allowlist** (per-tool meta-PRs that don't need a soak): `scripts/staging/`, `scripts/ci/check-staging-evidence(.test).ts`, `scripts/ci/lib/`, `scripts/gcp-setup/`, `docs/staging/`, `.github/workflows/staging-evidence.yml`, `CLAUDE.md`, `HANDOFF.md`, `.gitignore`, `.claude/settings.json`, `.claude/hooks/`, `package.json`, `package-lock.json`, `agents.md`.
...
- **`check-npm-install-policy.ts`** — blocks `npm ci` / `npm install` in GitHub Actions workflows, shell deploy helpers, and Dockerfiles unless lifecycle scripts are suppressed with `--ignore-scripts` or a nearby `install-scripts-ok:` comment gives an explicit exception reason.
