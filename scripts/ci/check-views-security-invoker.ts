...
const VIEW_NAME = String.raw`(?:"[^"]+"|\w+)`;
const CREATE_VIEW_REGEX = new RegExp(
  String.raw`CREATE\s+(?:OR\s+REPLACE\s+)?(?!MATERIALIZED\s+)VIEW\s+${SCHEMA_PREFIX}(${VIEW_NAME})`,
  'gi',
);
const SECURITY_INVOKER_KEY = String.raw`(?:"security_invoker"|\bsecurity_invoker\b)`;
...
const ALTER_FIX_REGEX = new RegExp(
  String.raw`ALTER\s+VIEW\s+(?:IF\s+EXISTS\s+)?${SCHEMA_PREFIX}(${VIEW_NAME})\s+SET\s*\([^)]*${SECURITY_INVOKER_KEY}\s*=\s*${SECURITY_INVOKER_TRUE_VALUE}[^)]*\)`,
  'gi',
);
const ALTER_UNFIX_REGEX = new RegExp(
  String.raw`ALTER\s+VIEW\s+(?:IF\s+EXISTS\s+)?${SCHEMA_PREFIX}(${VIEW_NAME})\s+(?:SET\s*\([^)]*${SECURITY_INVOKER_KEY}\s*=\s*${SECURITY_INVOKER_FALSE_VALUE}[^)]*\)|RESET\s*\([^)]*${SECURITY_INVOKER_KEY}[^)]*\))`,
  'gi',
);
const SECURITY_INVOKER_WITH_REGEX = new RegExp(
  String.raw`\bWITH\s*\([^)]*${SECURITY_INVOKER_KEY}\s*=\s*${SECURITY_INVOKER_TRUE_VALUE}[^)]*\)`,
  'i',
);
