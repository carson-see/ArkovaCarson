...
- **`reconcile-migration-ledger.ts`** — dry-run-first Supabase production migration ledger reconciliation helper. Prints or applies guarded SQL to rewrite timestamp-format ledger versions to Arkova numeric migration prefixes.
- **`reconcile-migration-ledger.test.ts`** — colocated safety tests for ledger matching, ambiguity refusal, SQL generation, and apply confirmation.
...
- Production ledger writes require dry-run review and explicit `--apply --confirm-project-ref <ref>`.
