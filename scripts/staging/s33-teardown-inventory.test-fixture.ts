...
export const TEST_TEARDOWN_DECLARATION = {
  ...
  rigs: [
    ...
    },
    {
      ...
      cloudRunServiceNames: ['arkova-worker-test-only-rig-r-staging'],
      schedulerJobNames: ['arkova-worker-test-only-rig-r-staging-test-trigger'],
      perRigSecretNames: [
        'supabase-url-test-only-rig-r-staging',
        'supabase-service-role-key-test-only-rig-r-staging',
      ],
    },
  ],
...
const targetSecretNames = TEST_TEARDOWN_DECLARATION.rigs.flatMap(
  (rig) => [...rig.perRigSecretNames],
);
  ...
  return {
    ...
    resources: {
      supabaseProjects: [
        ...
        { ref: 'zabcdefghijklmnopqrs', name: 'protected-shared-project' },
      ],
      cloudRunServices: [
      ...
      secretNames: [
        ...targetSecretNames,
        ...TEST_TEARDOWN_DECLARATION.protectedSharedSecretNames,
        'protected-unrelated-secret',
      ],
    },
  } as const;
}
...
export function testTeardownAfterInventory(
  ...
  const before = testTeardownBeforeInventory();
  return {
    ...before,
    phase: 'after',
    capturedAt,
...
export function buildTestTeardownCapturedVerification(options: {
  ...
  const after = captureS33TeardownInventory(
    TEST_TEARDOWN_DECLARATION,
    testTeardownAfterInventory(afterCapturedAt),
    {
      ...afterMetadata,
