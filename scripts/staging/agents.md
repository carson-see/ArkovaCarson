...
Live admission now fails before paid project creation unless the image is digest-pinned, the explicit source HEAD equals the checkout, the soak ID is bounded, the tier is T1/T2/T3 with its 2h/12h/48h minimum, and the base SHA equals the checkout HEAD/origin-main merge-base. Gemini admission also binds the requested GCP project and endpoint to the existing `STAGING_APPROVED_GCP_PROJECT` identity contract and accepts only `projects/<approved-project>/locations/us-central1/endpoints/<numeric-id>`; foreign projects, other regions, model/publisher paths, and non-numeric endpoint IDs fail before mutation. Gcloud mapping values reject delimiter/control injection. After deploy, one Revision JSON observation binds the controller-resolved image digest, source label, exact non-secret critical env, and absence of `GEMINI_TUNED_RESPONSE_SCHEMA`; apply admission always records the newly captured Supabase ref. `clean_mirror` evidence accepts only the preflight's exact allowlisted schema, all six mandatory unique checks, the captured project ref, and a UTC timestamp; persisted evidence omits diagnostic details. Every declared Scheduler trigger is re-observed PAUSED after seed + preflight before any cadence update/resume. The apply suite is fully stubbed and creates no infrastructure. The exact five-file change (`scripts/agents.md`, this file, and `provision-isolated-rig.sh` plus its shell and TypeScript tests) is T0 tooling and may proceed through lane review and merge once CI and required review gates are green. A real operational rig/soak is not a prerequisite for this tooling PR; it is required later, before any runtime/soak evidence or S3.3 operational-readiness claim is made from the provisioner.
