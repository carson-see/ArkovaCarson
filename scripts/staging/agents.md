...
| `s33-429-attribution.ts` | **S3.3 Wave 2 five-bucket attribution contract (SCRUM-2707 / SCRUM-2793).** Builds one strict per-arm evidence record from pathname-normalized client 429s, exact correlation-ID limiter-log joins, and `ai_upstream_http_error` events. Query/fragment suffixes are stripped before persistence. Upstream retries coalesce only through explicit retry-loop attempt identity: attempts must be unique per correlation ID, contiguous from 1, and bounded at 3; duplicates, gaps, a fourth attempt, or a reused correlation ID starting again at attempt 1 fail the whole artifact closed without timestamp inference. Pins surface/model/region/v6/schema/MIME provenance, requires `Retry-After`, keeps all five buckets separate with no total, reports the unmounted per-org limiter structurally, and rejects unknown/raw body, key, PII, prompt, JWT, or secret fields. Pure/offline; it does not call staging or cloud APIs. |
