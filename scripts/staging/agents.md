...
## Parallel-rig reservation-ledger validator (SCRUM-2906 R1, 2026-07-21)

- `check-rig-reservations.ts` (`npx tsx scripts/staging/check-rig-reservations.ts [docs/staging/rig-reservations.json]`) + `check-rig-reservations.test.ts`. Pure `validateLedger(ledger)` (no I/O) + CLI. Reads the reservation ledger `docs/staging/rig-reservations.json` and fails closed (exit 1) on any **double-booking or malformed active row**; exit 2 on unreadable/unparseable ledger.
- Rules (only `status:"active"` rows participate; `example`/`released` are inert): `active-required-field` (active row must carry `reservation_id`, `rail`, `rig.cloud_run_service`, `rig.supabase_ref`), `double-booked-cloud-run-service`, `double-booked-supabase-ref` (the Supabase ref is the real isolation boundary per §1.11A — tag URLs isolate revisions only), `duplicate-reservation-id` (across all rows), `ledger-shape`.
- **Scaffold / design-only (NO live rigs):** stands up nothing, touches no rig, reads only the ledger file. NOT wired as a gating CI job under the PI-0.5 W3 freeze. Full procedure: `docs/staging/parallel-rig-provisioning-runbook-2026-07-21.md` (bakes in the verified worker Zod crash-loop env checklist from `services/worker/src/config.ts`). Tests: `npx vitest run scripts/staging/check-rig-reservations.test.ts` (13 cases, red-first).
