...
| File | Purpose |
|---|---|
| `fullsoak-daily-probes.sh` | Daily behavioral probes for the 2026-08 SOC 2 fullsoak (P1–P8: health, flags, verify, QR, API-key scope + revocation, anon-RPC sweep). First committed 2026-08-12 with the FD-P7 fix; the live soak session had been running it uncommitted, and the live copy stays frozen for the window. P7 mints a probe key through the real POST flow, exercises scope, revokes (asserting `revoked_at` is stamped — P7g), proves the revoked key is refused (P7d, CC6.8 daily), and deletes its own key (P7e). P7f is the FD-P7 regression assertion: list rows must expose an addressable `id`. `PROBE_KEY_CAP` (default 8) is an accretion backstop only. **The rig at soak start still runs the pre-fix worker: expect P7d/P7f FAIL until the post-freeze deploy; 4 accreted `soak-daily-revocation-probe-*` keys need out-of-band cleanup after the window.** |
...
| `s33-429-attribution.ts` | **S3.3 Wave 2 five-bucket attribution contract (SCRUM-2707 / SCRUM-2793).** Builds one strict per-arm evidence record from pathname-normalized client 429s, exact correlation-ID limiter-log joins, and `ai_upstream_http_error` events. Query/fragment suffixes are stripped before persistence. Upstream retries coalesce only by the worker-generated per-`withRetry` UUID; observed attempts must be unique, strictly increasing, and bounded at 3, while sparse sets remain valid because non-429 attempts are absent. Client correlation reuse cannot collapse invocations. Monthly-quota evidence requires the keyed `X-RateLimit-Limit: 1000` header. Pins surface/model/region/v6/schema/MIME provenance, requires `Retry-After`, keeps all five buckets separate with no total, reports the unmounted per-org limiter structurally, and rejects unknown/raw body, key, PII, prompt, JWT, or secret fields. Pure/offline; it does not call staging or cloud APIs. |
| `s33-load-plan.ts` | **S3.3 Wave 3 Lane 2 deterministic load-plan contract (SCRUM-2699 / SCRUM-2790 / SCRUM-2794).** Pure/offline planning for seeded open arrivals at an asserted 500/hour fixture and 5,000/hour ceiling, exact 90/10 tenancy across at least 30 organizations, at least four JWT-shard labels, a monthly-key lane, byte-boundary and AU/KE/OOD synthetic cases, five-minute observation/connector cadence, 2,999/3,001 Trigger-B oscillation, and the separate endpoint-eviction rollback scenario. Admission enumerates every exact generated profile and fails closed unless every rolling 60-second source-IP and JWT-shard window stays strictly below the declared limiter ceiling. The real prod-shape replay remains unavailable until a digest-pinned in-window baseline exists. It stores credential labels only and performs no I/O. |
| `s33-load-runner.ts` | **S3.3 Wave 3 Lane 2 injectable open-arrival kernel.** Runs an absolute arrival scheduler concurrently with an independent observation controller, so neither response nor poll latency can close the arrival loop or create a catch-up burst. SLO, Sentry, connector-pressure, and heartbeat adapters return strict bounded metadata and pinned artifact digests. The first control breach or anonymous-IP `429` header signal wakes both schedulers, prevents every later dispatch, and records the exact last dispatched sequence. Dispatch, sleep, and control-adapter failures also stop both schedulers before the run rejects; no partial evidence is returned. Unknown body/secret fields fail closed. No HTTP, filesystem, credential, GCP, Sentry SDK, rig, or production adapter is included. |
| `s33-load-evidence.ts` | **S3.3 Wave 3 Lane 2 evidence composer (SCRUM-2707 / SCRUM-2793).** Recomputes exact plan, runner, attribution, and per-pass artifact bindings; accepts only a complete schedule or the exact contiguous first-stop prefix; requires anonymous-IP stop signals to have the final correlation-bound header/log attribution join; derives five-minute non-injected error windows from dispatched results; imports the frozen five 429 buckets without summing them; and keeps per-org/payer telemetry structurally excluded. Fixture output is always `DEFERRED_POST_WAVE3`, never release evidence. |
| `s33-pt-sizing.ts` | **S3.3 Wave 3 Lane 2 PT/GSU memo contract (SCRUM-2708).** Provides an empty post-Wave-3 memo template plus validated throughput/headroom/GSU and comparable-window cost arithmetic. Live inputs must be measured or asserted from dated, digest-pinned HTTPS sources; unit fixtures are forced to use synthetic labels and can never masquerade as measurements, sourced pricing, spend, or a CTO decision. |
