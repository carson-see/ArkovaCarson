...
| `rig-target.ts` (+ `.test.ts`) | Shared `resolveRigTarget()` prod hard-block + `<ref>.supabase.co` validator + `ALLOWED_STAGING_PROJECT_REFS` allow-list, reused by the targeted-soak harnesses below. Standalone copy of #1463's `batch-drain-harness-lib` guard so those harnesses don't depend on that unmerged branch. Pure, unit-tested (prod-ref block, attacker-host `<ref>.tld` hole, allow-list). |
| `confirmation-proof-fault-harness.ts` (+ `-lib.ts`/`.test.ts`, `npm run staging:cpf-fault`) | **Targeted soak driver for #1408.** Seeds SECURED anchors (app-tree branch set, `block_header` NULL) on an isolated rig, points the rig's inclusion-proof stub at each fault class (`X-Stub-Fault` header) and drives `POST /jobs/populate-confirmation-proofs`, asserting the row classifies per the #1408 contract: TRANSIENT (5xx/429/timeout/network/ECONNRESET) → `pending`/retry; DEFINITIVE (4xx/RPC-application/reorg) → `stale`. The classification LOGIC is unit-proven red-first in `services/worker/src/jobs/confirmation-proof-fault-driver.test.ts` (drives the REAL `fetchConfirmationProof`); this harness is the HTTP-layer soak of the SAME taxonomy. `-lib` (`buildFaultPlan`/`verdictFor`/`isBoundedBackoff`) is pure + unit-tested. Prod-blocked (`rig-target`), isolated-tag-only API base, `--dry-run`, `--cleanup`. MUTATES a rig — throwaway isolated rig only, never a live soak. |
| `classify-backcatalog-harness.ts` (+ `-lib.ts`/`.test.ts`, `npm run staging:classify-backcatalog`) | **Targeted soak driver for #1410/#1427.** Seeds a mixed back-catalogue (all four proof classes, deliberately uneven so a collapsed census is caught) on an isolated rig **with migration 0354 applied** (asserted via an `anchor_proofs.proof_class` preflight — refuses a rig missing 0354, since a census there would falsely report zero), then drives `/jobs/classify-proof-backcatalog` through the five invariants generic load never hit: GUC-off → zero rows; census sum-consistency + exact seed match; resume/checkpoint (no double-count/skip); concurrency mutex; per-org scoping. Invariants (incl. negative controls) are unit-proven red-first in `services/worker/src/jobs/classify-backcatalog-driver.test.ts`. `-lib` (`defaultSeedMix`/`assertCensusMatches`/`classifyScopeKey`) is pure + unit-tested. Prod-blocked, isolated-tag-only, `--dry-run`, `--cleanup`. |
