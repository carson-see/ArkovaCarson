...
| `ai-soak-harness.ts` | **AI-path load generator** for the AI T3 soak (SCRUM-2383 re-tiered T2→T3). Drives the LIVE `POST /api/v1/ai/extract` + `/ai/template` + `/ai/tags` endpoints at a configurable `--rate` (req/hr; default 5000) using the vendored AI-01 golden corpus (`ai-eval/golden-cpe-cle-s3.json`). `/api/v1/ai/*` require a **Supabase user JWT** (`STAGING_AI_JWTS`), NOT an API key; `aiRateLimiter` = 30 req/min/user so the harness **shards across ≥4 JWTs** for ≥5k/hr and `planRate()` fails loud on an undersized pool. Writes a per-endpoint p50/p95/p99 + 429/transport-error evidence file (`--evidence-out`). Run: `npx tsx scripts/staging/ai-soak-harness.ts …`. |
| `ai-eval-gate-runner.ts` | **Live SCRUM-2382 (AI-02) eval-gate runner.** Continuously (every `--interval` min over `--duration`) samples the 48-entry golden GATE split through the LIVE `/ai/extract` endpoint, scores field F1 with the vendored scorer, and enforces aggregate weighted F1 ≥ 0.80 + per-field floors (creditHours ≥ 0.85, issuedDate ≥ 0.80, credentialType ≥ 0.80). Appends one JSONL record/round (verdict, weighted F1, per-field P/R/F1, sample misclassifications, extraction-error count). `--require-live` refuses to certify a round as merge-grade if the server-reported provider is `mock`/`fast-fallback` (i.e. rig missing `GEMINI_API_KEY`). Fail-closed exit code. Run: `npx tsx scripts/staging/ai-eval-gate-runner.ts …`. |
| `ai-eval/` | Shared AI-soak library: `scoring.ts` (vendored, behaviour-equivalent port of `services/worker/src/ai/eval/scoring.ts` + the SCRUM-2382 gate from #1413's `eval-gates.ts` — so the soak's F1 equals the merge gate's), `golden-cpe-cle-s3.json` (vendored 60-fixture AI-01 golden set snapshot from #1413 @ `b95851d5`, all synthetic/PII-free), `golden.ts` (loader), `ai-client.ts` (auth + HTTP), `rate.ts` (sharding/pacing), `harness-core.ts` (stats/payloads), `eval-core.ts` (score→gate→record). All pure-logic modules have `*.test.ts` (red-first). Re-vendor when #1413's golden set / gate config changes. |
