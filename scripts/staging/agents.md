...
| `s33-429-attribution.ts` | **S3.3 Wave 2 five-bucket attribution contract (SCRUM-2707 / SCRUM-2793).** Builds one strict per-arm evidence record from pathname-normalized client 429s, exact correlation-ID limiter-log joins, and `ai_upstream_http_error` events. Query/fragment suffixes are stripped before persistence. Repeated upstream attempts inheriting one inbound correlation ID become one request-level event with a frozen, bounded attempt list; exact duplicate attempts still fail closed. Pins surface/model/region/v6/schema/MIME provenance, requires `Retry-After`, keeps all five buckets separate with no total, reports the unmounted per-org limiter structurally, and rejects unknown/raw body, key, PII, prompt, JWT, or secret fields. Pure/offline; it does not call staging or cloud APIs. |
