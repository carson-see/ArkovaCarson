...
const DEFAULT_STAGING_PROJECT_REF = 'ujtlwnoqfhtitcmsnrpq';
const PROD_PROJECT_REF = 'vzwyaatejekddvltxyye';
const STAGING_PROJECT_REF = process.env.STAGING_SUPABASE_PROJECT_REF ?? DEFAULT_STAGING_PROJECT_REF;
...
async function seedProfiles(client: LooseClient, orgs: SeededOrg[]): Promise<SeededProfile[]> {
  ...
  for (const org of orgs) {
    ...
    for (let u = 0; u < n; u++) {
      ...
      profiles.push({ id, org_id: org.id, email });
    }
  }
  ...
  await ensureProfilesExist(client, profiles);
...
async function ensureProfilesExist(client: LooseClient, profiles: SeededProfile[]): Promise<void> {
  const existing = new Set<string>();
  const PROFILE_LOOKUP_CHUNK = 100;
  for (let i = 0; i < profiles.length; i += PROFILE_LOOKUP_CHUNK) {
    const ids = profiles.slice(i, i + PROFILE_LOOKUP_CHUNK).map((p) => p.id);
    ...
    if (error) throw new Error(`profiles existence check failed at chunk ${i / INSERT_CHUNK}: ${error.message}`);
    for (const row of data ?? []) existing.add(row.id);
  }

  const missing = profiles.filter((p) => !existing.has(p.id));
  ...
  const inserted = await bulkInsert(client, 'profiles', rows, { onConflict: 'id' });
  console.log(`  inserted ${inserted} profiles directly (auth trigger absent on fresh public-schema baseline)`);
}
