...
Read-only drift audit between the required secrets the Cloud Run worker is currently bound to (`.github/workflows/deploy-worker.yml` `--set-secrets`) and the SEC-HARDEN-02 expected-secrets contract. Optional tooling-only secrets are reported separately and do not fail the production gate.
...
20/22 required bound. 2 drift.
0/1 optional bound.
```

## Adding a new expected secret

1. Add production-required env vars to `EXPECTED_SECRETS` in [`audit-env.ts`](./audit-env.ts). Add tooling-only or disabled-provider env vars to `OPTIONAL_SECRETS`.
