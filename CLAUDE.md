...
> **🧹 VERTEX COST CLEANUP (2026-04-16):** 62 idle endpoints undeployed + 63 empty endpoint shells deleted. **3 endpoints remain**: v5-reasoning prod (`8811908947217743872`), v6 ready (`740332515062972416`), fraud-v1 (`2117308101131501568`). Waste categories purged: 6× v6 intermediate checkpoints, 6× v6-compliance-pro (never eval'd), 12× v5-extraction-deep, 9× golden-2026-04-04, 8× golden-2026-04-03, 8× golden-2026-03-29, etc. **New rule**: undeploy any test Vertex endpoint in the same session; monthly audit `gcloud ai endpoints list` should show ≤5 deployed endpoints max.
> **📋 GEMINI GOLDEN v7 PLAN (2026-04-16):** Design doc `docs/plans/gemini-golden-v7-design-2026-04-16.md`. Goal: fix v6's two weaknesses (sparse-type F1 collapse + confidence regression). Three orthogonal bets: (1) **+155 hand-curated golden entries** across 7 weak types (IDENTITY, REGULATION, TRANSCRIPT, RESUME, BADGE, MILITARY, CHARITY — 20-25 each); (2) **retrain isotonic calibration layer** in `services/worker/src/ai/eval/calibration.ts` — no base-model retrain, $0 cost; (3) **add Vertex `responseSchema`** to `callTunedModel` for belt-and-suspenders JSON enforcement. Same training config as v6 (flash, 6 epochs, ADAPTER_SIZE_FOUR — ceteris paribus so we isolate dataset effect). DoD: Macro F1 ≥80%, **per-type F1 ≥75% for ALL types** (new bar), confidence Pearson r ≥0.5 (up from v6's 0.117). Cost ~$40, ~1 day. Blocked on v6 cutover first.
> **🔬 v6 STRATIFIED EVAL FINDING (2026-04-16) — v6 is much stronger than 50-sample eval suggested:** Ran n=10/type stratified eval (249 entries) via new `--stratified` flag in `run-eval.ts`. **Macro F1 79.3%** (up from 77.1% proportional), **19 of 23 types already ≥75%**. The "weak types" in the 50-sample were n=1-3 noise: TRANSCRIPT 63.9→90.6%, REGULATION 57.8→86.5%, IDENTITY 55.6→81.8%, BADGE 68→89.1%, MILITARY 50→77.6%. **True weaknesses (confirmed n=10):** RESUME 53.1% (real training gap) and ACCREDITATION 42.9% (data-hygiene: 20 entries in `golden-dataset-phase14.ts` mislabeled vs v6 canonicalizer). Borderline: FINANCIAL 70.6%, LEGAL 73.1%, MEDICAL 73.6%, CHARITY 74.6%. **v7 scope revised: 190 entries (vs 375), surgical targets only** — 30 RESUME + 15×4 borderline types + 100 fraud seed + 20 ACCREDITATION fix. Derived v6 calibration knots project **79.8% calibrated mean vs 78.3% actual (1.4pp gap, down from 24pp)**, ECE 24%→~5%. Pearson r stuck at 0.26 because flash raw confidence clusters narrowly — isotonic lifts mean, doesn't spread ranking; may need `adjustedConfidence` meta-model as the real discriminator. See `services/worker/docs/eval/eval-gemini-golden-v6-stratified-2026-04-16.md` + SCRUM-772 comment 11060.
> **🧹 VERTEX COST DISCIPLINE (2026-04-16):** Rule: **only deploy endpoints when actively training or serving.** Current state: **2 deployed** (v5-reasoning prod serving, v6 kept through v7 cutover as rollback target). Fraud-v1 undeployed (idle). Post-v7: **1 deployed endpoint** (v7 serving prod); v5-reasoning + v6 + fraud-v1 all undeployed (artifacts preserved for emergency redeploy). Flash-tuned endpoints bill per-request not per-hour-idle, so the cost discipline is about quota + operational hygiene, not raw $.
...
| **GME2 (Gemini Golden v6/v7)** ★ ACTIVE | **0/5** | **0** | **5** | **0% — v6 trained, cutover pending** |
