...
**There is no standing shared rig — provision an isolated one per soak.** The former `arkova-staging` standalone project (`ujtlwnoqfhtitcmsnrpq`) **no longer exists**: it was deleted, its DNS does not resolve, and the `supabase-url-staging` secret is an orphaned pointer. The Cloud Run service `arkova-worker-staging` still exists but is a **zombie** — it boots against a database that is gone. Target neither. In particular do **not** run `npx supabase db push --linked` "against shared staging": `--linked` resolves to whatever project is linked, and there is no standing project to link.

The live mechanism is **`scripts/staging/provision-isolated-rig.sh`** — one command that creates a standalone Supabase project (`us-east-2`, NOT a preview branch), replays the schema, deploys a wired `arkova-worker-<name>-staging` Cloud Run service on the prod-pinned image, seeds the baseline fixture, and requires `clean_mirror` from the preflight before returning. It is `--dry-run` by default; a live run needs `--apply` plus a matching `CONFIRM_PROVISION`, and non-mock profiles additionally need `CONFIRM_REAL_CONFIG`. Prod and the dead shared-staging refs are hard-denied inside the script. Migrations apply to **your** rig — link it, then `npx supabase db push --linked`. Tear the rig down with `scripts/staging/teardown-isolated-rig.sh` when the soak closes (§7 cost sweep). Full operations doc: [docs/reference/STAGING_RIG.md](./docs/reference/STAGING_RIG.md).
...
Cloud Run tag URLs isolate worker revisions only. They do not isolate Supabase schema, migration ledger rows, seed data, queues, cron side effects, audit rows, or soak evidence. Two PRs may share one rig only when they can truthfully share one clean database state. Any PR that changes migrations, RLS, schema, cron behavior, queue/batch semantics, seed assumptions, or other mutable DB state needs its **own** isolated Supabase project plus a separately wired `*-staging` Cloud Run service — which, with no standing shared rig left, is the default path for essentially every T3.
