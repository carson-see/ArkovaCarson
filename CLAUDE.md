...
No code, migration, RLS policy, cron, edge function, or env change reaches prod without first being applied to the staging environment + validated against the staging worker at the proportional tier. **Standing rig:** `arkova-staging` is a **standalone Supabase project** (project_ref `ujtlwnoqfhtitcmsnrpq`, region `us-east-2`) + `arkova-worker-staging` Cloud Run — NOT a Supabase preview branch. Apply migrations to staging via `npx supabase db push --linked`. Full operations doc: [docs/reference/STAGING_RIG.md](./docs/reference/STAGING_RIG.md).
...
Staging evidence is valid only when the database/project used for the soak is clean for that PR. Before starting any T2/T3 soak, run `scripts/ci/staging-honesty-preflight.ts` against the exact Supabase project ref that the worker will use and capture the result. Shared staging evidence from `ujtlwnoqfhtitcmsnrpq` is merge-grade only when the preflight reports `environment_type=clean_mirror`. `soak_artifact`, `fixture_seeded`, timestamp migration rows, duplicate migration names/versions, unexplained prod divergence, or PR-only ledger rows make that project invalid for new merge evidence until reconciled.

Cloud Run tag URLs isolate worker revisions only. They do not isolate Supabase schema, migration ledger rows, seed data, queues, cron side effects, audit rows, or soak evidence. Parallel shared-staging soaks are allowed only when the PRs can truthfully share one clean database state. Any PR that changes migrations, RLS, schema, cron behavior, queue/batch semantics, seed assumptions, or other mutable DB state needs either exclusive use of clean shared staging or an explicitly approved isolated Supabase project plus a separately wired `*-staging` Cloud Run service.

Do not start, restart, or claim a soak on a dirty project. If contamination is found mid-soak, stop the clock, mark that evidence invalid, and either cleanly rebuild with explicit approval or move to an isolated project. Do not repair/delete/insert migration ledger rows, run `migration repair`, run `supabase db push --linked`, or reset shared staging to make evidence look clean unless Carson/operator explicitly approves the exact operation.
