...
> Two rules, always:
>
> 1. **Never merge a PR to `main`.** Ever. Commit code on a branch, open a PR for review, and stop. Carson and the human reviewers own the merge. If your task feels like it needs a merge to finish, it is done when the PR is green and awaiting review — not when it lands on `main`. This mirrors the hard rule in `.claude/projects/.../memory/feedback_never_merge_without_ok.md` ("Open PRs freely; NEVER merge without explicit user OK").
> 2. **Always get caught up before coding in a new session.** At the start of every session — before editing any file — read, in this order: (a) the full CLAUDE.md (this file) including the version line + stats header, (b) `docs/BACKLOG.md` (the single source of truth for open work), (c) `HANDOFF.md` (current state + open blockers + decisions), (d) the `agents.md` in every folder you're about to touch, and (e) `docs/SARAH_BACKLOG.md` (your curated task list — excludes Nessie and Gemini Golden). Only start coding once you can state, in a sentence, what the last session shipped and what you are picking up next. If the task is about an existing PR or story, also read the Jira ticket and any recent comments before writing code.
>
> Sarah's backlog lives at `docs/SARAH_BACKLOG.md`. It intentionally excludes every Nessie track (NPH, NTF, NDD, NSS, NVI, NMT) and every Gemini Golden track (GME, GME2-8) — those belong to the AI-platform track and are NVI-gated. Stick to the list.

> **Version:** 2026-04-21 | **Repo:** ArkovaCarson | **Deploy:** app.arkova.ai (arkova-26.vercel.app) | **Network:** MAINNET ONLY (testnet/signet retired)
> **🩹 2026-04-21 TOP-10 UAT BUG-FIX SPRINT (batch 1 + 2 — 13 bugs resolved):** 8 open UAT bugs closed in batch 1 (commit 23098598) + 5 more in batch 2 (commit b11c4787). **Batch 1:** (1) `/compliance/scorecard` 403 for individuals → short-circuit the fetch + render org-required empty state via new shared `OrgRequiredCard`. (2) `/settings/api-keys` engineering copy "Ensure the worker service is running." removed; `useApiKeys` + `useApiUsage` now accept `{ enabled }` so individuals skip worker round-trips entirely. (3) `/settings` DataCorrectionForm 404 flood on missing `data_subject_requests` gated on strict-truthy `VITE_ENABLE_DSAR_UI`. (4) Public verify page raw ISO-8601 metadata now auto-formatted in `CredentialRenderer.formatFieldValue`. (5) 5 stale `arkova.io` refs → `.ai` in `arkova-marketing/` + `infra/cloudflare/load-balancer.ts`. (6) macOS Finder duplicates `services/worker/src/lib/urls 3.ts` + `urls.test 3.ts` removed. (7) `AuthGuard` voluntary-sign-out toast regression test. (8) `OrgRequiredCard` extracted to `src/components/shared/`. **Batch 2:** (9) Public `/search` silent-catch blocks now `console.error` for triage. (10) `/contact` now routes on `search.arkova.ai`. (11) `errorSanitizer` scrubs `worker service` phrasing. (12) `ApiUsageDashboard` no longer leaks raw error strings to users. (13) Dead `@ts-ignore` on `read-excel-file/browser` removed. 1,321 frontend tests + 21 sanitizer tests green + typecheck clean + `lint:copy` clean. All 13 entries logged under BUG-2026-04-21-001..013 in [bug_log.md](./docs/bugs/bug_log.md).
> **Stats:** 212 migrations (0000-0221 — prod synced through 0221, including 0159/0187/0188/0189/0190/0191 (partial, 2 of 3 BRIN)/0193/0195/0196/0197/0216/0219/0220/0221 applied via Supabase MCP during the 2026-04-19 + 2026-04-20 sprints. **Deferred to maintenance window: `idx_brin_anchors_created` (BRIN index on 2.8M-row anchors table timed out mid-build via MCP; existing btree `idx_anchors_user_created` covers the workload).** New in 2026-04-20 sprint: 0221 `get_agents_for_user` SECURITY DEFINER RPC (MCP security fix — closes cross-org `list_agents` data leak)) | 4,552 tests (1,321 frontend + 3,208 worker + 23 new sanitizer tests — +76 from 2026-04-21 top-10 UAT sprint regression coverage, +73 from Top-10 Sprint Batch 4 scaffolding modules 2026-04-18) | **355 stories** (338 complete + 3 NCA/DRIFT follow-ups (SCRUM-906/907/908) + 1 MCP-SEC epic (SCRUM-918) + 6 MCP-SEC children (SCRUM-919–924) + 1 NCA-FU2b (SCRUM-925) filed 2026-04-20). **2026-04-19 UAT sprint:** systematic click-through of every user-facing page on `arkova-26.vercel.app` at 1280 + 375 — caught **BUG-2026-04-19-001** (Dashboard UsageWidget stuck in loading skeleton for every user with large anchor row counts; anchors-count query through RLS timed out at 30s + 500 → React Query retry-stacked 90s+ stuck state; fixed via 5s AbortController timeout + graceful fallback in PR #426; verified fixed in prod post-deploy). Remaining Blocked (8 stories) are genuinely external: pentest vendor (SCRUM-517), SOC 2 auditor (SCRUM-522), Kenya ODPC filing (SCRUM-576/577), GEO marketing launches (SCRUM-477/478/479), SEC IAPD API replacement (SCRUM-727), NCA follow-up (SCRUM-893). **NPH-16 (SCRUM-728) deployed 2026-04-17; Cloud Run revision `arkova-worker-00322-m26` live at 100% traffic (2026-04-18 redeploy)** | 24/24 audit + 9 pentest findings resolved | AI: **Gemini 2.5 Flash (prod extraction, v5-reasoning tuned `endpoints/8811908947217743872`, single deployed Vertex endpoint)** — v6/v7 tuned and eval'd but did NOT cut over (v7 failed DoD 2026-04-16 PM, v7.1 surgical retrain planned); Nessie v27.3 FCRA UNDER_REVIEW + v28.0 HIPAA + v29.0 FERPA **QUARANTINED** (NVI-15) | 1.41M+ public records | 1.41M+ SECURED anchors (mainnet)
...
> **🧪 TOP-10 SPRINT BATCH 4 SCAFFOLDING (2026-04-18 — PR #436):** NVI-aware scaffolding for the next Nessie training tranche. Ten deterministic modules + 73 new tests under `services/worker/scripts/intelligence-dataset/{nph,kau,ntf}/` encoding the acceptance criteria for SCRUM-711 (NPH-14 v8 retrain eval gates), SCRUM-753/754 (KAU-05/06 Kenya + Australia credential types + NDB sources), SCRUM-773..779 (NTF-01..07 reasoning quality scorer, LoRA ablation winner selector, compliance-Q&A eval, cross-ref verification, credential portability, regulatory-conflict resolver, audit-finding validator). All pure offline logic — no LLM calls, no tuning jobs submitted, NVI gate still active. Confluence: `Top-10 Sprint Batch 4 — 2026-04-18`.
...
_Directive version: 2026-04-20 (MCP security audit + NCA-FU2 MVP sprint, PR #455) | 212 migrations (0221 applied) | 4,476 tests | **355 stories** (338 complete + SCRUM-906/907/908 + MCP-SEC epic SCRUM-918 + children SCRUM-919–924 + NCA-FU2b SCRUM-925) | GME 20/20 | NCE 20/20 | NMT 14/14 | **NVI 18/18** | **NCA-FU1 6/6** | **NCA-FU2 MVP shipped 2026-04-20 (SCRUM-906): MyRecordsPage deep-link auto-opens dialog with pre-filled credential_type** | **API-RICH 5/5** | **INTL 6/6** | **TRUST 2/7** | **MCP security hardening 2026-04-20 (PR #455): list_agents cross-org leak closed via `get_agents_for_user` SECURITY DEFINER RPC; Zod format validators tightened; HMAC marketing claim stripped; error messages redacted; stale CORS origin fixed. 6 follow-up stories filed under SCRUM-918.** | **2026-04-20 dep bumps merged/pending:** react-router-dom 7 (pending), read-excel-file 9, vite 8, eslint 10, eslint-plugin-react-hooks 7 (compiler rules downgraded to warn), @vitejs/plugin-react 6 (pending), production-deps group (supabase-js 2.104, sentry 10.49, tanstack-query 5.99, esbuild 0.28, typescript 6.0.3), worker-deps group (pending), typescript 6 edge + worker (pending), wrangler 4.84, @cloudflare/workers-types, @huggingface/transformers 4 | **Closed — needs migration story:** Tailwind 3→4 (full CSS-first rewrite, not a drop-in bump; eslint-plugin-react-hooks v7 compiler-rule cleanup; calibration-refit proper view migration per TODO in services/worker/src/jobs/calibration-refit.ts) | Nessie v27.3 FCRA / v28.0 HIPAA / v29.0 FERPA deployed; v27.3 UNDER_REVIEW + v28/v29 QUARANTINED per NVI-15 | Gemini v5-reasoning prod, v6 cutover pending, v7 failed DoD | Major remaining: Tailwind 4 migration, react-hooks-7 rules cleanup, REG (26 not started), TRUST-01/02/03, pentest + SOC 2 external, `idx_brin_anchors_created` maintenance-window apply_
