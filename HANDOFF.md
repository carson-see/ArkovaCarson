...
_Last refreshed: 2026-04-27 by claude — claims verified against vitest/CI output and `.github/workflows/ci.yml` (SCRUM-1259/1262 + SCRUM-1273/1269 batch: vitest run output shows 132 + 54 = 186 tests green across `anchor-stats`/`mainnet-migration`/`batch-anchor`/`batch-anchor.audit`/`utxo-provider`/`anchor-submit`/`aiFeatureGate`/`usageTracking`/`account-export`/`perOrgRateLimit` suites; `npx tsc --noEmit` in services/worker exits 0; `npm run lint` returns pre-existing warnings 0 errors all tenant-isolation per SCRUM-1208; `npm run lint:copy` returns "No forbidden terms found"; `SCAN_ALL=1 npx tsx scripts/ci/feedback-rules/no-aws.ts` returns "✅ feedback_no_aws: no AWS imports detected"; PR #567 dda518f confirmed in main via `git log --grep` query result, R2-1..R2-5 awaiting Jira transition)._
