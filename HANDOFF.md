...
CI status: SonarCloud PASS, CodeRabbit PASS. GitHub Actions stalled 2026-05-08 20:54Z (spending limit). Resumed 2026-05-09. Fixed pre-existing E2E strict-mode violation in `secure-document.spec.ts:190` (two buttons matched `/Copy Verification Link/i`; switched to `exact: true`). Staging evidence check verified locally (all 12 T3 fields present).
