...
**A real §1.12 T2 soak is still owed** and is OUT OF THIS PR's scope. It requires the `arkova-worker-staging` Cloud Run service (being built by a parallel session at the time of this commit) running against the staging DB under synthetic load — anchors, cron cycles, E2E flows — to verify the schema works under application behavior, not just object existence. When the staging worker is up, the genuine soak runs and the staging-evidence section here gets the real numbers. Until then, the entries below are honest: **schema check only, not behavior check.**
...
* **PR #700 not yet merge-ready.** As of last check: `MERGEABLE | BLOCKED` with `reviewDecision: CHANGES_REQUESTED` from CodeRabbit. CI re-running on the rebased + post-fix commits — many checks pending. Findings addressed in commit `43073507`: pgcrypto schema fix (codex-bot P1), README index-claim clarification, cutover doc TBD-placeholder cleanup, bash brace expansion fix, sonar full exclusions promoted from cpd-only to full exclusions for migrations.
...
* **Pre-baseline 0290** (PR #697) is captured in the baseline (since 0290 was already applied to prod when the dump was taken). PR #697's 0290 file is a no-op idempotent re-apply on top of the baseline — drop from #697's diff or keep as 0291 on top after PR #695's 0291 lands.
