...
**Prod state verified this session:** Cloud Run worker is serving revision `arkova-worker-00558-mgb` at 100% latest-revision traffic with `DOCUSIGN_INTEGRATION_KEY`, `DOCUSIGN_CLIENT_SECRET`, `DOCUSIGN_CONNECT_HMAC_SECRET`, `ENABLE_DOCUSIGN_OAUTH=true`, `ENABLE_DOCUSIGN_WEBHOOK=true`, and `DOCUSIGN_DEMO=true`. `/health` returned 200 with `git_sha=6899f10aba7e233755385edfd2b28112129e41d7`, `network=mainnet`. Safe prod smoke on 2026-05-15 passed invalid HMAC `401 invalid_signature` and signed unknown account `200 orphaned`. Prod has one enabled SCRUM-1655 sandbox `ESIGN_COMPLETED` rule and 0 active `provider=docusign` integrations; prod accepted+duplicate smoke is paused at DocuSign MFA until the sandbox OAuth callback creates the active integration.
