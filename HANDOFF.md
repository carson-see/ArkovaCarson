...
### 2026-05-05 — SCRUM-1668 Path C baseline / PR #700 status corrected

PR [#700](https://github.com/carson-see/ArkovaCarson/pull/700) is **not merge-ready**. The prod ledger row for `00000000000000 / baseline_at_main_HEAD` is recorded, and the schema-only equivalence artifact is captured at [docs/staging/PATH_C_VERIFICATION_2026-05-04.md](./docs/staging/PATH_C_VERIFICATION_2026-05-04.md). That artifact includes the `pg_class`, `pg_indexes`, `pg_proc`, and `information_schema` checks plus the ledger `INSERT ... RETURNING` output.

This is deliberately narrower than a behavior soak: the evidence proves the baseline can recreate prod's schema shape on an empty branch project, not that the worker has run against it. A real worker/staging validation path is still owed before #700 is Done: Docker worker build/run and/or `arkova-worker-staging` against an approved staging DB under representative load. Shared staging ownership remains active in parallel #695/#697 work, so #700 should wait for the lease or use an explicitly approved isolated environment.

This branch now also fixes the local CI assumptions that broke on the pg_dump baseline: local Postgres images without `index_advisor`, pg_dump inline quoted constraints, baseline quota-exception text, the prod-table snapshot drift, and the old `switchboard_flags` seed shape. The baseline file and `docs/migrations-archive/` remain one atomic migration-history rewrite; do not split them.
