...
### 2026-05-12 — dependency PR consolidation ready for human review

**Routine dependency batch — PR #772 (`codex/deps-routine-20260512`) replaces #764, #770, and #771.**
Root/worker/edge routine bumps are bundled into one PR with the generated `org_credits` type-map repair and a drop-search overload test cleanup required by the stricter lint/type surface. Local gates passed: root typecheck/lint/lint:copy/license/build/test, worker typecheck/lint/build:circuit/test/build, edge typecheck. Staging T1 evidence is captured at `docs/staging/soak-pr-772-20260512T1339Z.json`: Cloud Run tag URL `https://pr-772---arkova-worker-staging-kvojbeutfa-uc.a.run.app`, deploy log id 14, 30-minute read-only soak 1497/1497 ok, p95 140ms, 0 failures. GitHub checks are green, migration drift is green, CodeRabbit approved, and merge state is clean. Superseded PRs #764/#770/#771 are closed. Remaining step is human merge.

**Frontend major dependency batch — PR #773 (`codex/deps-frontend-major-20260512`) replaces #767, #768, and #769.**
React/React DOM and type packages are upgraded to 19.x, Tailwind is migrated from v3 config to v4 CSS-first `@theme` tokens plus `@tailwindcss/postcss`, and deprecated v3 utilities are migrated. Local gates passed: `npm run typecheck`, `npm run lint`, `npm run lint:copy`, `npm run security:license-denylist`, `npm run build`, and full `npm run test` (205 files, 1988 tests). Local Playwright built-preview smoke covered `/dashboard`, `/compliance/scorecard`, `/records`, and `/privacy` at 1280x900 and 375x812 with zero console/page errors; protected routes redirected cleanly to login without `.env.test` auth storage, while authenticated coverage is represented by the green GitHub E2E suite. GitHub checks, Vercel previews, migration drift, staging evidence, Lighthouse, and CodeRabbit are green. SCRUM-915 Confluence page v2 now records AC/DoD evidence and Jira is moved from To Do to Needs Human. Superseded PRs #767/#768/#769 are closed. Remaining step is human approval/merge. PR #753 and PR #774 were not reset, reseeded, edited, or otherwise touched.
