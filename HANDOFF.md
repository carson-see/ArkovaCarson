...
**Draft PR #783 is still open/draft; do not merge.** Runtime staging is now pinned to worker `BUILD_SHA=1a1667da7a509ecb3c4a930e07e02ce271b0f1c7` after the cycle-1 cold-start bugfix. Staging tag URL is `https://pr-783---arkova-worker-staging-kvojbeutfa-uc.a.run.app` on worker revision `arkova-worker-staging-00089-zal`, image `us-central1-docker.pkg.dev/arkova1/arkova-worker-images/arkova-worker:pr783-1a1667da`, digest `sha256:4c01da1c05dc1cd67a30e139e3dcb466a0f07386c8183f763f5ba3fa3bb4b7b2`, Cloud Build `dd58876e-6b8d-4101-905b-854e73e60c0f`, deploy log id `37`, Supabase ref `ujtlwnoqfhtitcmsnrpq`. Flags on the revision: `ENABLE_DOCUSIGN_WEBHOOK=true`, `ENABLE_ORG_CREDIT_ENFORCEMENT=true`, `ENABLE_RULES_ENGINE=true`, `ENABLE_RULE_ACTION_DISPATCHER=true`, `USE_MOCKS=true`, `ENABLE_PROD_NETWORK_ANCHORING=false`, `BATCH_ANCHOR_MAX_SIZE=100`. Health check at 2026-05-15T14:37:26Z returned 200 with matching `git_sha`.
...
**T3 soak restarted on the fixed runtime and is incomplete.** The accepted fixed-runtime soak window starts 2026-05-15T14:37:26Z and must run at least 48h, through 2026-05-17T14:37:26Z, covering the 2026-05-16 and 2026-05-17 03:00 America/New_York scheduler cycles. Daily scheduler config observed: `daily-anchor-flush`, `0 3 * * *`, timezone `America/New_York`, target `/jobs/batch-anchors?force=true`. The active thread heartbeat `scrum-1658-pr783-3am-cycles` is set for 03:05 Eastern to run the isolated org-scoped scheduled fixture (`org_id=00001658-0783-4000-8000-000000000105`) on the PR #783 tag only, without touching PR #753/#781/Nessie/#774/SCRUM-908/SCRUM-1803/package/deploy protected areas.

**Pre-restart scheduler cycle 1 evidence (2026-05-15 03:05 America/New_York):** pre-state was scheduled org credits `2`, `ARK-DOC-TMD3CH` `PENDING`, no `organization_queue_runs`. First isolated POST to `https://pr-783---arkova-worker-staging-kvojbeutfa-uc.a.run.app/jobs/batch-anchors?force=true&org_id=00001658-0783-4000-8000-000000000105` at 2026-05-15T07:07:58Z returned 500 `Processing failed`; Cloud Run logs show cold-start race `getChainClient() called before initChainClient()`. Health/startup completed at 2026-05-15T07:09:40Z (`Using MockChainClient`, `Chain client initialized`). A 2026-05-15T07:10:10Z retry collided with the worker's in-process batch mutex and returned 200/processed 0. Final isolated retry at 2026-05-15T07:12:24Z returned 200 with `processed=1`, `batchId=batch_1778829145276_1`, `merkleRoot=eeee...eeee`, `txId=mock_receipt_1778829145276_59d972`. Current DB query shows `ARK-DOC-TMD3CH` is `SECURED` as of 2026-05-15T07:14:31Z, with metadata `queue_credit_source=org_credits`, `queue_credit_reason=rule.auto_anchor_queue_run`, `queue_credit_balance_after=1`; org credits `2 -> 1`; `credit_transactions_count=0`. Per PR #774 rules, this is anchored because it is `SECURED` (not merely `SUBMITTED`). Cloud Run error-like summary for the cycle: 2 entries for the same cold-start failure (platform ERROR + app log level 50); no additional error after successful retry. This cycle is bug evidence only and does not count as a clean fixed-runtime T3 cycle.

**Next fixed-runtime scheduler fixture prepared:** new signed DocuSign envelope `scrum1658-pr783-scheduled-env-20260515T071321Z` returned 202 with rule event `435680af-dd4e-41ed-b9c8-40a403e3a79d`; `/jobs/rules-engine` processed 1/1 and `/jobs/rule-action-dispatcher` dispatched 1/1. Anchor `ARK-DOC-QD8KYF` is `PENDING`, credits remain `1`, and it is left for the 2026-05-16 03:00 America/New_York fixed-runtime scheduler cycle. After that cycle, create one more scheduled fixture for the same org and leave it `PENDING` for the 2026-05-17 03:00 America/New_York cycle.

**Cold-start bugfix evidence:** commit `1a1667da` made `initChainClient()` idempotent, made `getChainClientAsync()` lazily initialize on demand, and moved batch-anchor chain calls to the async getter. Red/green coverage added for cold-start forced flushes and fresh-module lazy async init. Verification before deploy: `npx vitest run src/jobs/batch-anchor.test.ts src/jobs/batch-anchor.audit.test.ts src/chain/client.test.ts src/jobs/org-queue-scheduler.test.ts src/routes/cron.test.ts` = 5 files / 241 tests passing; `git diff --check` passed; earlier affected checks `npm run typecheck`, `npm run build`, and ESLint on touched worker files passed.
