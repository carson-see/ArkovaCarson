...
**Prod state verified this session:** Cloud Run worker was manually updated after Carson approval to revision `arkova-worker-00556-m5l` with `DOCUSIGN_INTEGRATION_KEY`, `DOCUSIGN_CLIENT_SECRET`, `DOCUSIGN_CONNECT_HMAC_SECRET`, `ENABLE_DOCUSIGN_OAUTH=true`, `ENABLE_DOCUSIGN_WEBHOOK=true`, and `DOCUSIGN_DEMO=true`. `/health` returned 200 with `git_sha=89e967f1d5a4cc9ccbcc536f69d31ed427e4ebdd`, `network=mainnet`. Safe prod smoke on 2026-05-15 passed invalid HMAC `401 invalid_signature` and signed unknown account `200 orphaned`. Prod still has 0 active `provider=docusign` integrations and 0 DocuSign/ESIGN rules, so prod accepted+duplicate smoke is not possible without adding a real prod integration/rule.
