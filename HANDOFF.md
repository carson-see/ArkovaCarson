...
**Tests:** PR #712 final after Codex P1/P2 + CodeRabbit fixes ships with 29 SCRUM-1755-surface tests (17 resolver including 2 query-error cases + 6 IssueCredentialForm + 2 SecureDocumentDialog title + 4 useIssueCredentialSplit). Broader area sweep across `src/components/{anchor,organization,upload,credentials,vault}`, `src/hooks`, `src/lib/copy*` returns 374/374 green. `tsc --noEmit -p tsconfig.json` returns 0 errors; `npm run lint:copy` reports no forbidden terms; `eslint` on touched paths reports 0 errors / 0 warnings. Verification artifacts: PR #712 CI run (link populated by `gh pr checks 712` once green), and `npx vitest run` output captured in PR body.
...
* Follow-up: regen `src/types/database.types.ts` so the migration 0289 `suspended` column drops the eslint-disable + cast in `useCanIssueCredential.fetchOrgGateRow()`.

_Last refreshed: 2026-05-05 by claude (Sarah session) — claims verified against `npx vitest run`, `npx tsc --noEmit`, `npm run lint:copy` outputs in worktree at commit 2d5858c6+pending fixups._
