...
dscribe('SCRUM-1086 — get_org_members_public', () => {
  ...
  // SCRUM-1301 fix: the previous tests seeded a user into ORG_IDS.arkova and
  // searched the first 200 members. The RPC orders by `ord_name = public_id`
  // ascending; seeded `99999999-…` sorts to the end. If Arkova has > 200
  // seeded members locally, the seeded test user falls outside `LIMIT 200`
  // and `find()` returns undefined → flake. Fix: each test creates its own
  // sandbox org so the seeded user is the only member returned.
  ...
  it('private-profile members are anonymized: display_name "X. Surname", avatar+id null', async () => {
    ...
    await (serviceClient as any).from('organizations').upsert({
      id: sandboxOrgId,
      legal_name: 'SCRUM-1086 Private Sandbox',
    });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it('public-profile members get full payload + their own profile_public_id', async () => {
    ...
    await (serviceClient as any).from('organizations').upsert({
      id: sandboxOrgId,
      legal_name: 'SCRUM-1086 Public Sandbox',
    });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (serviceClient as any).from('profiles').upsert({
      id: testUserId,
